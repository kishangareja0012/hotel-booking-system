...
const mutations = {
  ...
  setapaleoPropertyKey(state, newValue) {
    state.apaleoPropertyKey = newValue;
  },
...
const actions = {
  ...
  async getAuthUrl({ commit }) {
    try {
      const { data } = await ApiService.post(`${apiEndpoint}/apaleo/connect`, { object_id: initialProperty });
  ...
  async apaleoObjectMap() {
    try {
      await ApiService.post(`${apiEndpoint}/apaleo/object-map`, { apaleoPropertyKey: state.apaleoPropertyKey, object_id: initialProperty });
    } catch (error) {
      handleApiError(error);
    }
  },
  setapaleoPropertyKey({ commit }, newValue) {
    commit('setapaleoPropertyKey', newValue);
  },
  async revokeApaleoConnection() {
    try {
      await ApiService.post(`${apiEndpoint}/apaleo/revoke`, { object_id: initialProperty });
    } catch (error) {
      handleApiError(error);
    }
  },
  async checkConnection({ commit, dispatch }) {
    try {
      const { data } = await ApiService.post(`${apiEndpoint}/apaleo/status`, { object_id: initialProperty });
      let status = '';
      if (data.status === 1) {
        status = 'yes';
      } else {
