...
const getters = {
  ...
  contractPlans: (state, getters) => (getters.loaded ? state.data.contractPlans : []),
  contractRooms: (state, getters) => (getters.loaded ? state.data.contractRooms : []),
  ...
  settingsPropertyId: (state) => state.settingsPropertyId || null,
};
...
const mutations = {
  ...
  promoCreated(state, { contract, contractPlans }) {
    state.updatePending = false;
    state.data.channel.contractor.codes.push(contract);
    state.data.contractPlans.push(...contractPlans);
  },
  ...
  getProperty(state, data) {
    ...
    state.settingsPropertyId = propertyData.id;
    state.pending = false;
  },
  getColor(state, { data }) {
    [state.colors] = data;
...
const actions = {
  ...
  async fetchContractData({ commit }, { id, forced = false, contract = 'contract' } = {}) {
    commit('beforeLoading', forced);
    try {
      const { data } = await ApiService.get(`${endpoint}/${id}/contract/${contract}`);
      commit('data', { data });
    } catch (error) {
      commit('data', { error });
      throw handleApiError(commit, error);
  ...
  async createContract({ commit }, { id, promo }) {
    ...
    try {
      ...
      const payload = { ...promo };
      delete payload.mode;
      const { data: { contract, contractPlans } } = await ApiService.post(`${endpoint}/${id}/${mode}`, payload);
      commit('promoCreated', { contract, contractPlans });
    } catch (error) {
  ...
  async fetchProperty({ commit }, propertyId) {
    ...
    try {
      let { data } = await ApiService.get(`${endpoint}/${propertyId}/property`);
      if (!data.length) {
        const propertyData = {
          code: propertyId,
          defaultLanguage: StorageService.getLang(),
        };
        const createdData = await ApiService.post(`${endpoint}/property`, propertyData);
        data = [createdData.data];
      }
      commit('getProperty', data);
    } catch (error) {
      commit('data', { error });
    }
  },
    ...
    try {
      ...
      const pickupEmailItem = (langId) => {
        ...
        const email = {
          ...
          hotel: `/hotels/${state.settingsPropertyId}`,
        };
    ...
    try {
      ...
      const pickupEmailItem = (langId) => {
        ...
        const email = {
          ...
          language: `/languages/${langId}`,
          project: `/projects/${defaultProjectId}`,
          property: `/properties/${state.settingsPropertyId}`,
        };
