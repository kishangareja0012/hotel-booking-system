...
const getters = {
  ...
  propertyKey: (state, getters) => (getters.loaded ? state.propertyKey : ''),
...
const actions = {
  async fetchData({ commit, dispatch }, { forced = false } = {}) {
    ...
    try {
      if (state.currentId === systemList.apaleo.id) {
        let { data } = await ApiService.post(`${endpoint}/apaleo/rate-plans`, { object_id: initialProperty });
  ...
  async connectRatePlans({ commit, dispatch }, { propertyId = null, forced = true }) {
    ...
    try {
      if (state.currentId === systemList.apaleo.id) {
        await ApiService.put(`${endpoint}/apaleo/product-map`, { selected, object_id: initialProperty, property_id: propertyId });
      } else if (state.currentId === systemList.hostaway.id) {
  ...
  async setPropertyKey({ commit, dispatch }, newValue) {
    commit('setPropertyKey', newValue);
    dispatch('objectMap');
  },
  ...
  async objectMap({ dispatch }) {
    try {
      await ApiService.post(`${endpoint}/apaleo/object-map`, { apaleoPropertyKey: state.propertyKey, object_id: initialProperty });
  ...
  async hostawaySetClientSecret({ commit }, hostawayAccountID) {
    try {
      commit('beforeUpdate');
      const { data } = await ApiService.post(`${apiEndpoint}/hostaway/client-secret`, { object_id: initialProperty, account_id: hostawayAccountID.hostawayAccountID, api_key: hostawayAccountID.hostawayAPIKey });
