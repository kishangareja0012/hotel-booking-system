...
const routes = [
  ...
  {
    path: '/join/users/:id?/properties/:property_id?/:hash?',
    name: 'users.property.join',
    meta: {
      title: 'Accept to join',
      centered: true,
      perms: false,
      translationKey: 'userPropertyJoin',
    },
    component: () => import(/* webpackChunkName: "user" */ '@/views/User/JoinProperty.vue'),
  },
  {
    path: '/details',
    name: 'details',
  ...
  },
  {
    ...
    meta: {
      title: 'Dashboard',
      documentTitle: '*dashboard.pageTitle',
  ...
  },
  {
    ...
    meta: {
      title: 'Reservations',
      documentTitle: '*reservations.pageTitle',
  ...
  },
  {
    ...
    meta: {
      title: 'Meal plans',
      documentTitle: '*mealPlans.pageTitle',
  ...
  },
  {
    ...
    meta: {
      title: 'Promotions',
      documentTitle: '*promotions.title',
  ...
  },
  {
    ...
    meta: {
      title: 'Settings',
      documentTitle: '*pages.settings.title',
  ...
  },
  {
    ...
    meta: {
      title: 'Properties',
      documentTitle: '*pages.properties.title',
      empty: true,
...
const initialPageByPerms = () => {
  ...
  if (perms.includes('properties')) return 'properties';
  if (perms.includes('users')) return 'users';
  if (perms.includes('group')) return 'group';
  return 'noperms';
};

const initialPageForProperty = () => {
...
const getInitialRoute = () => {
  ...
  const hasProperties = $store.getters['user/hasProperties'];
  return hasProperties ? initialPageForProperty() : initialPageByPerms();
};

// const updateTranslations = async (to) => {
//   const commonTranslations = ['validation', 'links', 'datepicker', 'date', 'buttons', 'langs', 'booking', 'legal',
//     'navbarTitles', 'restrictions', 'guestsPrices', 'channels', 'commonKeys', 'productsSelector', 'masterData',
//     'addr', 'resetPwd', 'permissions', 'groups', 'adminGroups', 'hotels', 'menu', 'dashboard', 'properties'];
//   if ([
//     'login', 'reservations', 'mealPlans', 'booking', 'calendar', 'roomTypes', 'ratePlans',
//      'masterData', 'mealPlans', 'contracts', 'contactPersons', 'nearby', 'facilities',
//      'policies', 'photos', 'channels',
//      'channel', 'systems', 'hotels', 'group', 'description', 'adminGroups', 'adminGroup',
//      'promotions', 'groups', 'users', 'channelshealth', 'invoices', 'setupWizard', 'profile', 'payments', 'settings',
//     'dashboard'].includes(to.meta.translationKey)) {
...
const updateTranslations = async () => {
  const menuNames = ['login', 'reservations', 'mealPlans', 'booking', 'calendar', 'roomTypes', 'ratePlans',
    'masterData', 'mealPlans', 'contracts', 'contactPersons', 'nearby', 'facilities', 'policies', 'photos',
    'channels', 'channel', 'systems', 'hotels', 'group', 'description', 'adminGroups', 'adminGroup', 'promotions',
    'groups', 'users', 'channelshealth', 'invoices', 'setupWizard', 'profile', 'payments', 'settings', 'dashboard',
    'validation', 'links', 'datepicker', 'date', 'buttons', 'langs', 'booking', 'legal', 'navbarTitles', 'restrictions',
    'guestsPrices', 'channels', 'commonKeys', 'productsSelector', 'masterData', 'addr', 'resetPwd', 'permissions', 'groups',
    'adminGroups', 'hotels', 'menu', 'dashboard', 'properties', 'feedbackForm'];
  const data = await ApiService.post(`${translationToolUrl}/gateway/translations`, {
    project: translationToolProject,
    pages: menuNames,
  });
...
router.beforeEach(async (to, from, next) => {
  ...
  const forEmptyProperties = to.matched.some((r) => r.meta.empty === true);
  ...
  const initial = hasProperties ? initialPageForProperty() : initialPageByPerms();
  ...
  if (!hasProperties && !forEmptyProperties) {
    return next({ name: initial });
  }
