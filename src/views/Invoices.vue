    ...
    <div class="panel position-relative panel-content">
      <div class="list d-none d-md-block">
        ...
        <div v-else>
          <p class="head-line justify-content-between">
            <span>{{ $t('invoices.heading') }}<spinner v-if="pending" /></span>
            <search-filter
              v-model="filterUnpaid"
              :disabled="pending"
              :placeholder="$t('invoices.filterByInvoice')"
            />
          </p>
      ...
      <div class="d-md-none list-item" v-for="row in filteredInvoices" :key="row.id">
        <div class="d-flex line">
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersBeneficiary') }}</p>
            <p>{{ row.beneficiary }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersInvoice') }}</p>
            <p>{{ row.invoice }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersProduced') }}</p>
            <p>{{ row.produced }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersPeriodFrom') }}</p>
            <p>{{ row.period_from }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersPeriodTo') }}</p>
            <p>{{ row.period_to }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('invoices.headersSum') }}</p>
            <p>{{ row.sum }}</p>
          </div>
        </div>
        <div class="line">
          <div class="w-100">
            <p class="label">{{ $t('invoices.headersDownload') }}</p>
            <p>
      ...
      invoices() {
        if (this.data) {
          return this.data.data.map((item) => ({
            ...
            produced: moment(item?.generatedDate).format(this.$t('invoices.format')),
            period_from: moment(item?.periodFrom).format(this.$t('invoices.format')),
            period_to: moment(item?.periodTo).format(this.$t('invoices.format')),
