...
<template>
  <div class="page-calendar" id="page-calendar">
    ...
    <b-modal
      ...
      :ok-disabled="dataUpdating || !$refs.quickUpdateForm"
    ...
    >
      ...
      <ValidationObserver ref="quickUpdateForm" slim>
        ...
        <div class="edge" :class="{ active: quickUpdate.selectedRooms.length }">
          <h3 :content="$t('calendar.selectRooms')"></h3>
          <div class="row">
        ...
        <div class="edge" :class="{ active: quHasMainFields }">
          <h3 :content="$t('calendar.whatToEdit')"></h3>
          <div class="row row-edit">
            <div class="col cell-edit-label">
              <check-box v-model="quickUpdate.do_avail"
                         :disabled="dataUpdating">{{ $t('calendar.avail') }}</check-box>
            </div>
          ...
          <div class="row row-edit">
            <div class="col cell-edit-label">
              <check-box v-model="quickUpdate.do_price"
                         :disabled="dataUpdating">{{ $t('calendar.price') }}</check-box>
            </div>
            <div class="col cell-edit-field">
              <ValidatedField
            ...
            <div class="col cell-edit-tip">
              {{ $t('calendar.priceTip', { currency: currency }) }}
            </div>
          </div>
          <div class="row row-edit">
            <div class="col cell-edit-label">
              <check-box v-model="quickUpdate.do_commission"
                         :disabled="dataUpdating">{{ $t('calendar.updateModalCommission') }}</check-box>
            </div>
            <div class="col cell-edit-field">
              <ValidatedField
                type="number" name="qucommission" min="0" max="99" class="mb-0" no-icon no-tooltip
                v-model="quickUpdate.commission" :disabled="!quickUpdate.do_commission || dataUpdating"
                :rules="rulesFor('commission')"
              />
            </div>
            <div class="col cell-edit-tip">
              {{ $t('calendar.updateModalCommissionTip') }}
            </div>
          </div>
          <div class="row row-edit">
            <div class="col cell-edit-label">
              <check-box v-model="quickUpdate.do_osale"
                         :disabled="dataUpdating">{{ $t('calendar.status') }}</check-box>
            </div>
            <div class="col-auto cell-edit-field">
              <switcher v-model="quickUpdate.osale" :disabled="!quickUpdate.do_osale || dataUpdating"/>
            </div>
            <div class="col cell-edit-tip">
              {{ $t(`calendar.status${quickUpdate.osale ? 'Open' : 'Closed'}`) }}
            </div>
          </div>
        </div>
        <div class="edge" :class="{ active: quHasMoreFields }">
          <h6 :class="{'opened': quMoreOpened}" @click="quMoreOpened=!quMoreOpened">
            {{ $t('calendar.changeMore') }}
          ...
          <div class="row">
            ...
            <div class="col-12 cell-restrictions">
              <drop-down
                ...
                multiple
                select-all
                title-all="calendar.restrictionsAll"
            ...
            >
              <div class="dates d-flex">
                <div class="month" v-for="month in dates" :key="month.month">
                  <span class="text-center"> {{ month.month }} </span>
    ...
    <div class="day-panels" v-if="periodSet && dataLoaded">
      <div class="panel position-relative panel-content"
        ...
        <div class="room-header">
          ...
          <span class="id ml-2">({{ $t('calendar.roomId', { id }) }})</span>
        </div>
        <div class="room-data d-flex">
          <div class="headers">
            ...
            <div class="header"
                 :class="{ 'last-shown': !selectedRestrictions.length }">{{ $t('calendar.roomPrice') }}
              <span class="asset" v-html="currencySymbol()"></span></div>
            <div class="header">{{ $t('calendar.roomCommission') }}</div>
          ...
          <div class="rows" ref="roomRows">
            ...
            <div class="data room-editable" data-field="avail">
              <div class="day" v-for="day in data[id]" @click.self="focusField($event)"
                ...
                <b-input :value="day.avail" v-else
                         ...
                         @change.capture="updateField($event, id)"
                />
              </div>
            </div>
            ...
            <div class="data room-editable" data-field="commission">
              <div class="day" v-for="day in data[id]" @click.self="focusField($event)"
                   :key="day.i" :class="day.avail>0 && !day.csale ? 'bookable' : 'blocked'">
                <spinner v-if="isDayUpdating(id, day.i, 'commission')"/>
                <div v-else-if="isDayUpdated(id, day.i, 'commission')" class="day-updated">
                  <icon width="18" height="18" type="ok-tick"/>
                </div>
                <b-input :value="+day.commission"
                         :disabled="day.avail==0"
                         @focus.prevent="selectField($event, day)"
                         @mouseup.stop.prevent
                         @blur="deselectField"
                         @keyup.esc="resetField"
                         @keyup.enter="updateField($event, id, day, 'commission')"
                />
              </div>
            </div>
            <div v-for="rts in restrictionOptions"
              ...
              <div class="day" v-for="day in data[id]" @click.self="focusField($event)"
                ...
                <template v-else>
                  <b-input class="booking_slot" v-if="!rts.cb"
                           :value="day[rts.fld]"
                           :disabled="day.avail==0"
                           @focus.prevent="selectField($event, day)"
                           @mouseup.stop.prevent
...
<script>
  ...
  export default {
    ...
    async created() {
      ...
      await this.setRoomdbIsMaster(this.property.roomdb_is_master);
      await this.syncRoomsWithRoomDB();
    ...
    computed: {
      ...mapGetters('user', ['currency', 'dateFormat', 'property']),
    ...
    methods: {
      ...mapActions('calendar', ['fetchRooms', 'fetchData', 'updateData', 'batchUpdateData', 'setRoomdbIsMaster', 'syncRoomsWithRoomDB']),
      ...
      async updateField(ev, id, selDay, selField) {
        ...
        if (field === 'avail') {
          ...
          data = {
            ...
            carr: dayObject.carr,
            cdep: dayObject.cdep,
            commission: dayObject.commission,
          };
          data[field] = value;
        }
        if (!cb) {
      ...
      async processQuickUpdate() {
        ...
        if (result) {
          ...
          $.each(this.dataUpdateError.response.data.errors, (key, value) => {
            this.$toastr.e(value[0], 'Error');
          });
        }
      },
