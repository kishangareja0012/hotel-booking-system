<template>
  <div class="page-widget-status">
    <div class="panel-title position-relative w-100 title">
      <h2>{{ $t('dashboard.settingTitle') }}</h2>
    </div>
    <div class="widget-status-group" v-if="!isAllhide(1)">
      <div class="widget-status-group-header">
        <h3 class="widget-status-group-header-title">{{ $t('dashboard.kpiTitle') }}</h3>
        <div class="widget-status-group-header-activate">
          ...
          <span v-else @click="toggleActivateAll(1)">{{ $t('dashboard.activate') }}</span>
        </div>
      </div>
      <div class="widget-status-group-content">
        <div class="widget-status-group-content-item" v-for="item in kpiGroupData" :key="item.widgetId">
          <switcher
            ...
            :on-label="$t('booking.on')"
            :off-label="$t('booking.off')"
    ...
    <div class="widget-status-group" v-if="!isAllhide(2)">
      <div class="widget-status-group-header">
        <h3 class="widget-status-group-header-title">{{ $t('dashboard.bookTitle') }}</h3>
        <div class="widget-status-group-header-activate">
          <span v-if="isActivateAllPerGroup(2)" @click="toggleActivateAll(2)">
            {{ $t('dashboard.deactivate') }}
          </span>
          <span v-else @click="toggleActivateAll(2)">{{ $t('dashboard.activate') }}</span>
        </div>
      </div>
      <div class="widget-status-group-content">
    ...
    <div class="widget-status-group" v-if="!isAllhide(3)">
      <div class="widget-status-group-header">
        <h3 class="widget-status-group-header-title">{{ $t('dashboard.ibeTitle') }}</h3>
        <div class="widget-status-group-header-activate">
          <span v-if="isActivateAllPerGroup(3)" @click="toggleActivateAll(3)">
            {{ $t('dashboard.deactivate') }}
          </span>
          <span v-else @click="toggleActivateAll(3)">{{ $t('dashboard.activate') }}</span>
        </div>
      </div>
      <div class="widget-status-group-content">
    ...
    <div class="widget-status-group" v-if="!isAllhide(4)">
      <div class="widget-status-group-header">
        <h3 class="widget-status-group-header-title">{{ $t('dashboard.channelTitle') }}</h3>
        <div class="widget-status-group-header-activate">
          <span v-if="isActivateAllPerGroup(4)" @click="toggleActivateAll(4)">
            {{ $t('dashboard.deactivate') }}
          </span>
          <span v-else @click="toggleActivateAll(4)">{{ $t('dashboard.activate') }}</span>
        </div>
      </div>
      <div class="widget-status-group-content">
    ...
    <div class="widget-status-group" v-if="!isAllhide(5)">
      <div class="widget-status-group-header">
        <h3 class="widget-status-group-header-title">{{ $t('dashboard.termTitle') }}</h3>
        <div class="widget-status-group-header-activate">
          <span v-if="isActivateAllPerGroup(5)" @click="toggleActivateAll(5)">
            {{ $t('dashboard.deactivate') }}
          </span>
          <span v-else @click="toggleActivateAll(5)">{{ $t('dashboard.activate') }}</span>
        </div>
      </div>
      <div class="widget-status-group-content">
    ...
    <!--    <div class="btn-group">-->
    <!--      <button class="btn-group-item" @click="onHide()">-->
    <!--        {{ $t('dashboard.setting.btn_cancel') }}-->
    <!--      </button>-->
    <!--      <button class="btn-group-item btn-save" @click="onSave()">-->
    <!--        {{ $t('dashboard.setting.btn_save') }}-->
    <!--      </button>-->
    <!--    </div>-->
    <button class="btn-close" @click="onHide()">X</button>
  </div>
</template>
