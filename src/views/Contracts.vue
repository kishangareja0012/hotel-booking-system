  ...
  <div class="page-promotions">
        ...
        <div class="row">
          <div class="col-12 col-md-6">
            ...
            <date-picker
              ...
              ref="promoFrom"
              :disabled="updatePending || promo.outdated"
              :placeholder="$t('date.fromPlaceholder')"
          ...
          <div class="col-12 col-md-6">
            ...
            <date-picker
              ...
              :disabled="updatePending || promo.outdated"
              :placeholder="$t('date.untilPlaceholder')"
    ...
    <div class="component-mealplans">
      <div class="panel position-relative panel-content">
        ...
        <p class="head-line head-line-md d-md-none">
          <span>{{ $t("contracts.heading") }}</span
          ><spinner v-if="!loaded" />
        </p>
        <promotion
...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('channel', ['loaded', 'channel', 'rooms', 'plans', 'contractPlans', 'contractRooms']),
      ...mapState('channel', ['error', 'pending', 'updatePending', 'updateError']),
      ...mapGetters('user', ['settings']),
      promoCreatePlans() {
        const filteredContractPlans = this.contractPlans.filter(({ promo }) => promo == null).pluck('id');
        return this.plans.filter(({ id }) => filteredContractPlans.includes(id.toString()));
      },
      promoModalTitle() {
        const {
    ...
    methods: {
      ...mapActions('channel', ['fetchContractData', 'createContract', 'updateContract', 'deleteContract']),
      ...mapMutations('channel', ['clearErrors']),
      async setChannelId(id) {
        this.id = id;
        try {
          await this.fetchContractData({ id, force: true });
        } catch (error) {
      ...
      promoItems() {
        ...
        codes = codes.filter((i) => i.mode === 'contract');
