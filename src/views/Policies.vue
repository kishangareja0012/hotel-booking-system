    ...
    <div class="page-cxl-pol">
      ...
      <div class="panel position-relative panel-content">
            ...
            <table class="w-100">
              <thead>
                <tr>
                  ...
                  <th class="w-name">{{ $t('policies.name') }}</th>
                  <th class="w-cxl-time">{{ $t('policies.time') }}</th>
                  <th class="w-cxl-fee">{{ $t('policies.fee') }}</th>
        ...
        <div class="d-md-none list-item" v-for="row in cxlPols" :key="row.id">
          ...
          <div class="d-flex line">
            <p class="label">{{ $t('policies.name') }}</p>
            <p class="c-name">{{ row.langs.en.name }}</p>
          </div>
          <div class="d-flex line">
            <p class="label">{{ $t('policies.time') }}</p>
            <p>{{ cxlTimeDesc(row.cancellationTime) }}</p>
          </div>
          <div class="d-flex line">
            <p class="label">{{ $t('policies.fee') }}</p>
            <p>{{ cxlFeeDesc(row.cancellationFee) }}</p>
          </div>
        </div>
      ...
      >
        ...
        <template #modal-title>
          {{ $t(`policies.${cxlPol.id != null ? 'edit' : 'add'}`) }}
        </template>
        <ValidationObserver ref="cxlPolForm" slim>
          ...
          <div class="edge" :class="{ active: edgeCPForm }">
            ...
            <div class="cp-fee-title">{{ $t('policies.sectionFee') }}</div>
            <div class="row row-edit row-cp-fee">
              <div class="col cell-edit-label">
                {{ $t('policies.feeRelation') }}
              </div>
    ...
    <div class="page-bgarant">
      <div class="panel position-relative panel-content">
        ...
        <div class="list d-none d-md-block left-edge">
          ...
          <div class="policies-table" v-if="loaded">
            <table class="w-100">
              <thead>
                <tr>
                  <th class="w-type">{{ $t('policies.bgarantHeadersType') }}</th>
                  <th></th>
                </tr>
              </thead>
        ...
        <div class="d-md-none list-item" v-for="row in bgarants" :key="row.id">
          <div class="d-flex line">
            <p class="label">{{ $t('policies.bgarantHeadersType') }}</p>
            <p>{{ row.title }}</p>
          </div>
        </div>
    ...
    <div class="page-pymt-pol">
      <div class="panel position-relative panel-content">
          ...
          <div class="policies-table">
            <table class="w-100">
              <thead>
                <tr>
                  <th class="w-id">{{ $t('id') }}</th>
                  <th class="w-name">{{ $t('policies.paymentHeadersName') }}</th>
                  <th class="w-pymt-time">{{ $t('policies.paymentHeadersTime') }}</th>
                  <th class="w-pymt-type">{{ $t('policies.paymentHeadersType') }}</th>
                  <th class="w-pymt-fee">{{ $t('policies.paymentHeadersFee') }}</th>
                  <th class="w-actions">{{ $t('actions') }}</th>
                </tr>
              </thead>
        ...
        <div class="d-md-none list-item" v-for="row in pymtPols" :key="row.id">
          ...
          <div class="d-flex line">
            <p class="label">{{ $t('policies.paymentHeadersName') }}</p>
            <p class="c-name">{{row.name}}</p>
          </div>
          <div class="d-flex line">
            <p class="label">{{ $t('policies.paymentHeadersTime') }}</p>
            <p>{{ pyTimeDesc(row.paymentTime) }}</p>
          </div>
          <div class="d-flex line">
            <p class="label">{{ $t('policies.paymentHeadersType') }}</p>
            <p>{{ pyTypeDesc(row.paymentType) }}</p>
          </div>
          <div class="d-flex line">
            <p class="label">{{ $t('policies.paymentHeadersFee') }}</p>
            <p>{{ pyFeeDesc(row.paymentFee) }}</p>
          </div>
        </div>
        ...
        <ValidationObserver ref="pymtPolForm" slim>
          <div class="edge active">
            ...
            <ValidatedField type="textarea" name="pp-desc" class="pp-desc"
                            :placeholder="$t('policies.paymentModalDescription')"
            ...
            <div class="row row-edit row-pp-type">
              <div class="col cell-edit-label">
                {{ $t('policies.paymentModalSectionType') }}
              </div>
            ...
            <div class="row row-edit row-pp-fee">
              <div class="col cell-edit-label smaller">
                {{ $t('policies.paymentModalSectionFee') }}
              </div>
              <div class="col cell-edit-field bigger">
                <amount-percent
            ...
            <div class="row row-edit row-pp-time">
              <div class="col cell-edit-label">
                {{ $t('policies.paymentModalSectionTime') }}
              </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      timeUnits() {
        return timeUnits.map((id) => ({ id, text: this.$tc(`policies.${id}`, 0) }));
      },
      dropTimes() {
        return dropTimes.map((id) => ({ id, text: this.$t(`policies.${id}`).capitalizeFirst() }));
      },
      basisTypes() {
        return basisTypes.map((id) => ({ id, text: this.$t(`policies.${id}`).capitalizeFirst() }));
      },
      paymentTypes() {
        return this.pmts.map((pmt) => ({
          ...pmt,
          text: this.$t(`policies.${pmt.text}`),
        }));
      },
      cancelFormValid() {
    ...
    methods: {
      ...
      cxlTimeDesc(ctu) {
        if (!ctu.unitMultiplier) {
          return this.$t('policies.cancelTimeNone');
        }
        const what = this.$tc(`policies.${ctu.timeUnit}`, ctu.unitMultiplier);
        const when = this.$t(`policies.${ctu.dropTime}`);
        return this.$t('policies.cancelTime', { what, when });
      },
      cxlFeeDesc(cf) {
        ...
        if (fs
          ...
          : cf.nmbrOfNights < 1) {
          return this.$t('policies.cancelFeeNone');
        }
        const percents = cf.mode !== 'amount';
        const { symbol } = this.currency;
        return this.$tc(`policies.cancelFee${cf.basisType}`,
      ...
      pyTimeDesc(ptu) {
        if (!ptu.unitMultiplier) {
          return this.$t('policies.paymentTimeNone');
        }
        return this.$tc(`policies.${ptu.timeUnit}`, ptu.unitMultiplier);
      },
      pyTypeDesc(code) {
        const pmt = this.pmts.find((bg) => (bg.id - code) === 0);
        return pmt != null ? this.$t(`policies.${pmt.text}`) : '';
      },
      pyFeeDesc(pyf) {
        const amt = pyf.mode === 'amount';
        if (pyf.value < (amt ? 0.01 : 1)) {
          return this.$t('policies.paymentFeeNone');
        }
        const { symbol } = this.currency;
        return this.$tc('policies.paymentFee',
