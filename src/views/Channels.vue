        ...
        <template #tab(available)>
          <div class="list">
            ...
            <div class="tablist-none" v-if="!availableChannels.length">
              {{ $t(`channels.${filter ? 'filterNone' : 'noAvailable'}`) }}
            </div>
            <div v-else class="plans-table">
              <table class="w-100 d-none d-lg-table">
                <thead>
                  <tr>
                    <th class="w-name">{{ $t('channels.headersName') }}</th>
                  ...
                  <tr>
                    ...
                    <td class="actions available text-right">
                      <div class="d-inline-block">
                        ...
                        >{{ $t('channels.buttonActivate') }}</b-btn>
              ...
              <mobile-table>
                <template #mobile>
                  <li v-for="row in availableChannels" :key="row.id">
                    <div class="row">
                      <div class="col-6">
                        <h6 class="font-weight-bold">{{ $t('channels.headersNameUrl') }}</h6>
                        <template v-if="row.url">
                          <a :href="channelUrl(row)" target="_blank">{{ row.name }}</a>
        ...
        <template #tab(connected)>
          <div class="list">
            ...
            <div class="tablist-none" v-if="!connectedChannels.length">
              {{ $t(`channels.${filter ? 'filterNone' : 'noConnected'}`) }}
            </div>
            <div v-else class="plans-table">
              <table class="w-100 d-none d-lg-table">
                <thead>
                  <tr>
                    <th class="w-name">{{ $t('channels.headersName') }}</th>
                    <th class="w-mapping text-center">{{ $t('channels.headersMapping') }}</th>
                    <th class="w-property-ids text-center">{{ $t('channels.headersIds') }}</th>
                ...
                <template #mobile>
                  <li v-for="row in connectedChannels" :key="row.id">
                    <div class="row">
                      ...
                      <div class="col-6 pb-4"
                           :class="($parent.collapsed && ! $parent.hovered) ? 'col-md-3' : 'col-md-6'">
                        <h6 class="font-weight-bold">{{ $t('channels.headersMapping') }}</h6>
                        <b-btn v-if="row.status !== 'inactive'"
                               class="btn-sm"
                      ...
                      <div class="col-6"
                           :class="($parent.collapsed && ! $parent.hovered) ? 'col-md-3' : 'col-md-6'">
                        <h6 class="font-weight-bold">{{ $t('channels.headersIds') }}</h6>
                        <icon width="16" height="16" type="info"
                              v-b-tooltip:page-channels.bottom="propertyIds(row.mapped)"
                              v-if="row.status !== 'inactive'"
                        />
                      ...
                      <div class="col-6"
                        ...
                        <h6 class="font-weight-bold">
                          {{ (row.status == 'active') ? $t('channels.headersStatus') : $t('actions') }}
                        </h6>
      ...
      <div class="panel position-relative panel-content" v-if="0">
        <div class="list d-none d-md-block">
          <p class="head-line justify-content-between">
            <span>{{ $t('channels.heading') }}</span><spinner v-if="!loaded" />
          ...
          <div class="plans-table" v-if="loaded">
            <table class="w-100">
              ...
              <tbody v-if="filter && !filteredChannels.length">
                <tr>
                  <td colspan="4" class="w-empty">{{ $t('channels.filterNone') }}</td>
                </tr>
              </tbody>
...
<script>
  ...
  export default {
    ...
    data: (vm) => ({
      fromChannel: false,
      tabs: [
        { id: 'connected', title: vm.$t('channels.tabsConnected') },
        { id: 'available', title: vm.$t('channels.tabsAvailable') },
      ],
      filter: '',
      cmap: {
        selected: [],
    ...
    computed: {
      ...
      ...mapGetters('user', ['propertyID', 'engineURL']),
      tab: {
      ...
      filteredChannels() {
        ...
        if (!filter) return this.channels.filter(({ id }) => (!`${id}`.includes(bookingEngineId)));
        return this.channels
          .filter(({ id, name }) => (`${id}`.includes(filter) || name.toLowerCase().includes(filter)))
          .filter(({ id }) => (!`${id}`.includes(bookingEngineId)));
      },
    },
    methods: {
      ...
      connectButtonText(channel) {
        ...
        const p = `channels.${!count ? 'none' : (count < total ? 'some' : 'all')}`;
