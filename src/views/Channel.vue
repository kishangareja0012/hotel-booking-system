  ...
  <div class="page-channel">
    ...
    >
      ...
      <ValidationObserver ref="promoForm" slim>
        <div class="row">
          <div class="col-12">
            <h5 class="pb-2">{{ $t(`channels.promoModalFieldName${promo.mode}`) }}</h5>
            <ValidatedField id="promo-name" name="name" no-icon
                            rules="required|min:3"
                            :placeholder="$t(`channels.promoModalFieldNamePlaceholder${promo.mode}`)"
                            v-model="promo.name" :disabled="updatePending || promo.outdated"/>
          </div>
        </div>
        <div class="row">
          <div class="col-12">
            <h5 class="pb-2">{{ $t(`channels.promoModalFieldCode${promo.mode}`) }}</h5>
            <ValidatedField name="code" no-icon
                            rules="required|min:3" :error-bag="updateError"
                            :placeholder="$t(`channels.promoModalFieldCodePlaceholder${promo.mode}`)"
                            v-model.trim="promo.code" :disabled="updatePending || promoIsEdit || promo.outdated"/>
          </div>
        </div>
        <div class="row" v-if="promo.mode === 'promo'">
          <div class="col-12">
            <h5 class="pb-2">{{ $t('channels.promoModalFieldDiscount') }}</h5>
        ...
        <div class="row">
          <div class="col-12">
            <h5 class="pb-4 pt-2">{{ $t('channels.promoModalFieldPlans') }}</h5>
            <products-selector
              v-if="!promoIsEdit"
              plans-only
        ...
        <h5 class="pb-2 pt-3">{{ $t('channels.promoModalFieldValidity') }}</h5>
        <div class="row">
          <div class="col-12 col-md-6">
            ...
            <date-picker
              ...
              @input="promoFromChanged" :format="dateFormat" />
          </div>
          <div class="col-12 col-md-6">
            ...
            <date-picker
              ...
              @input="promoUntilChanged" :format="dateFormat" />
          </div>
        </div>
      </ValidationObserver>
    </b-modal>
    <div>
      ...
      <div class="row" v-if="loaded && channel.type === 'push'">
        ...
        <div class="col">
          <div class="row" v-if="channel.period.number > 0">
            <div class="col cell-field">{{ $t('channels.connectTransmissionPeriod') }}</div>
            <div class="col cell-value">{{ periodValue }}</div>
          </div>
          <div class="row" v-if="channel.period.until != null">
            <div class="col cell-field">{{ $t('channels.connectTransmissionEndDate') }}</div>
      ...
      <floater v-if="channelIsValid && !isPullChannel" :shown="cmap.selected.length > 0">
        <template #content>
          <div class="floater-table">
            <div class="connect-heading">
              ...
              <p class="separator"></p>
              <p class="headline">{{ $t('channels.connectMyRatePlans') }}</p>
            </div>
            <div class="connect-table">
              <div class="connect-row" v-for="{ plan, cplan, uniq, mode, ctype, ptype } in cmap.selected"
                   :key="`cnn-${uniq}`">
                <div class="cell-dst">
                  <p class="room"><b>{{ $t('channels.connectMappedRoom') }}:</b>&nbsp;{{ ctype }}
                    ({{ cplan.typeid }})</p>
                  <p><b>{{ $t('channels.connectMappedPlan') }}:&nbsp;</b>{{ cplan.name }} ({{ cplan.id }})</p>
                </div>
                <div class="cell-link"><icon type="link" width="20" height="20"/></div>
                <div class="cell-src">
                  <p class="room"><b>{{ $t('channels.connectMappedRoom') }}:</b>&nbsp;{{ ptype.text }}
                    ({{ ptype.id }})</p>
                  <p><b>{{ $t('channels.connectMappedPlan') }}:&nbsp;</b>{{ plan.text }} ({{ plan.id }})</p>
                </div>
        ...
        <template #footer>
          <span class="connections-count">
            {{ $tc('channels.connectSelectedConnectionsCount', cmap.selected.length) }}
          </span>
          <b-btn pill variant="outline-primary" size="sm" :disabled="updatePending"
                 @click="connectPlans">{{ $t('channels.connectBtnConnect') }}</b-btn>
        </template>
      </floater>
      ...
      <floater v-if="channelIsValid && isPullChannel" :shown="hasPullChanges" no-content>
        <template #footer>
          <span class="connections-count">
            {{ $tc('channels.connectSelectedChangesCount', pullChangesCount) }}
          </span>
      ...
      <div class="d-flex d-md-none justify-content-center" v-if="searchShown">
        <search-filter v-model="filter" :disabled="!loaded || updatePending"
                       :placeholder="$t('channels.filterTip')" />
      </div>
      <tabs :items="tabs" v-model="tab" with-content
        ...
        <template #tab(pending) v-if="!isPullChannel">
          <div class="tablist-none" v-if="!pendingCTypes.length">
            {{ $t('channels.connectNoPending') }}
          </div>
          <div class="pending-table" v-else>
            <div class="dst-list">
              <p class="headline">{{ channel.name }}</p>
              <div class="rates-list" :class="{ active: seluniq != null }">
                <div class="type-item" v-for="{ name: tname, typeid, plans } in filteredRows"
                     :key="`ctype-${typeid}`">
            ...
            <div class="src-list" :class="{ 'd-none': seluniq == null }">
              ...
              <div class="rates-list">
                <div class="type-item" v-for="({ id, text, rates }) in pendingRooms" :key="`type-${id}`">
        ...
        <template #tab(connected) v-if="!isPullChannel">
          ...
          <div class="mapped-table" v-else>
            ...
            <table class="w-100">
              <tbody v-for="item in mappedPlansSearch" :key="`mapped-${item.uniq}`">
                <tr>
                  <td class="cell-dst">
                    <p class="room"><b>{{ $t('channels.connectMappedRoom') }}:&nbsp;</b>{{ item.ctype }}
                      ({{ item.cplan.typeid }})</p>
                    <p><b>{{ $t('channels.connectMappedPlan') }}:&nbsp;</b>{{ item.cplan.name }}
                      ({{ item.cplan.id }})</p>
                  </td>
                  <td class="cell-src">
                    <p class="room"><b>{{ $t('channels.connectMappedRoom') }}:&nbsp;</b>{{ item.room.text }}
                      ({{ item.room.id }})</p>
                    <p><b>{{ $t('channels.connectMappedPlan') }}:&nbsp;</b>{{ item.plan.text }}
                      ({{ item.plan.id }})</p>
                    <p>
                      <b-dropdown size="sm" toggle-tag="span" variant="link" no-caret dropup right
                                  :disabled="updatePending">
                  ...
                  <td class="cell-action">
                    ...
                    >{{ $t('channels.connectBtnDisconnect') }}</b-btn>
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div class="plans-heading">
              <p class="headline mb-3">
                <check-box :value="isAllMySelected" @input="toggleMyAll" :disabled="updatePending || !plans.length"
                           class="select-all">{{ $t('channels.mapModalCheckboxToggleAll') }}</check-box>
              </p>
            </div>
            <div class="all-plans">
              <products-selector
        ...
        <template #tab(urls)>
          <div class="urls-block">
            ...
            <b-form-row class="link-fields">
              ...
              <div class="col cell-link-tip">
                <a :href="urlMain" target="_blank">
                  {{ $t('channels.urlsOpenInNewTab') }}
                  <icon w="16" h="16" type="new-window" class="ml-1" />
                </a>
              </div>
            </b-form-row>
            <p class="mt-4">{{ $t('channels.urlsCustomLink') }}</p>
            <b-form-row class="link-fields">
              <div class="col cell-link-field copyable copy-icon">
            ...
            />
            <p v-else>{{ $t('channels.urlsNoMappedProducts') }}</p>
          </div>
        </template>
        <template #tab(promos)>
          <promotion
        ...
        <template #tab(settings)>
          <div class="settings-block">
            <div class="panel position-relative panel-content my-40">
              <div class="list left-edge" :class="{ 'px-0 pb-0': colorOpened }">
                <p class="head-line color-customization" :class="{ opened: colorOpened }"
                  ...
                  {{ $t('channels.settingsColorCustomizationTitle') }}
                ...
                <div>
                  <div class="d-flex color-switcher align-items-center">
                    <p :class="{ active: !isAdvanced }">
                      {{ $t('channels.settingsColorCustomizationLightMode') }}
                    </p>
                    <switcher medium v-model="isAdvanced" :disabled="updatePending"
                              class="switcher" />
                    <p :class="{ active: isAdvanced }">
                      {{ $t('channels.settingsColorCustomizationAdvancedMode') }}
                    </p>
                  </div>
                      ...
                      <ul class="advanced-menu w-100">
                        <li class="advanced-menu-item advanced-menu-item-dropdown"
                          ...
                          <a @click.prevent="toggleMenu('backgrounds')">
                            <span>
                              <span>{{ $t('channels.settingsColorCustomizationBackgrounds') }}</span>
                          ...
                          <div class="advanced-menu-dropdown">
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationMainBackground') }}
                              </p>
                              <drop-down
                              ...
                              <div class="color-form" v-show="colorCollection.template_background_type=='gradient'">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationTopColor') }}
                                </p>
                                <div class="d-flex">
                                  <span
                              ...
                              <div class="color-form" v-show="colorCollection.template_background_type=='gradient'">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationBottomColor') }}
                                </p>
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationHeaderBackground') }}
                              </p>
                              <drop-down
                            ...
                            <div class="color-option">
                              <div class="color-info">
                                <p class="color-title">
                                  {{ $t('channels.settingsColorCustomizationProductBackground') }}
                                </p>
                                ...
                                >
                                  <p class="popover-info"
                                     v-html="$t('channels.settingsColorCustomizationIncludeSections')" />
                                </b-popover>
                              </div>
                              <drop-down
                        ...
                        <li class="advanced-menu-item advanced-menu-item-dropdown"
                          ...
                          <a @click.prevent="toggleMenu('fonts')">
                            <span>
                              <span>{{ $t('channels.settingsColorCustomizationFonts') }}</span>
                              <icon class="arrow" stroke-width="2" width="12" height="7" type="arrow-down"/>
                            </span>
                          </a>
                          <div class="advanced-menu-dropdown">
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationLanguageSelection') }}
                              </p>
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationCheckColor') }}
                              </p>
                              <div class="color-form">
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationDateColor') }}
                              </p>
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationFontColor') }}
                                </div>
                                <div class="d-flex">
                                  <span
                                    id="date-color"
                              ...
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomization.background-color') }}
                                </div>
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <div class="color-info">
                                <p class="color-title">
                                  {{ $t('channels.settingsColorCustomizationProductName') }}
                                </p>
                                ...
                                >
                                  <p class="popover-info"
                                     v-html="$t('channels.settingsColorCustomizationTravelPeriod')" />
                                </b-popover>
                              </div>
                              <div class="color-form">
                                <div class="sub-title">
                              ...
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationFontStyle') }}
                                </div>
                                <div class="d-flex">
                                  <ValidatedField
                                    :disabled="updatePending"
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationMeals') }}
                              </p>
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationColorIncluded') }}
                                </div>
                                <div class="d-flex">
                                  <span
                              ...
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationColorExcluded') }}
                                </div>
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationBookingSummary') }}
                              </p>
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationFontColor') }}
                              ...
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationBackgroundColor') }}
                                </div>
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <div class="color-info">
                                <p class="color-title">
                                  {{ $t('channels.settingsColorCustomizationProductNote') }}
                                </p>
                                ...
                                >
                                  <p class="popover-info"
                                     v-html="$t('channels.settingsColorCustomizationSameChange')" />
                                </b-popover>
                              </div>
                              <div class="color-form">
                                <div class="sub-title">
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationProductAmount') }}
                              </p>
                              <div class="color-form">
                                <div class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationFontColor') }}
                        ...
                        <li class="advanced-menu-item advanced-menu-item-dropdown"
                          ...
                          <a @click.prevent="toggleMenu('buttons')">
                            <span>
                              <span>{{ $t('channels.settingsColorCustomizationButtons') }}</span>
                              <icon class="arrow" stroke-width="2" width="12" height="7" type="arrow-down"/>
                            </span>
                          </a>
                          <div class="advanced-menu-dropdown">
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationSearch') }}
                              </p>
                              <div class="color-form">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationFontColor') }}
                              ...
                              <div class="color-form">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationNormalState') }}
                                </p>
                                <div class="d-flex">
                                  <span
                              ...
                              <div class="color-form">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationMouseOver') }}
                                </p>
                                <div class="d-flex">
                                  <span
                            ...
                            <div class="color-option">
                              <p class="color-title">
                                {{ $t('channels.settingsColorCustomizationOtherButtons') }}
                              </p>
                              <drop-down
                              ...
                              />
                              <p class="sub-title">
                                {{ $t('channels.settingsColorCustomizationBackgroundNormal') }}
                              </p>
                              ...
                              <p class="sub-title">
                                {{ $t('channels.settingsColorCustomizationBackgroundMouse') }}
                              </p>
                              <div class="color-form" v-show="colorCollection.other_buttons_background_type=='flat'">
                                <div class="d-flex">
                                  <span
                        ...
                        <li class="advanced-menu-item advanced-menu-item-dropdown"
                          ...
                          <a @click.prevent="toggleMenu('inputFields')">
                            <span>
                              <span>{{ $t('channels.settingsColorCustomizationInputFields') }}</span>
                              <icon class="arrow" stroke-width="2" width="12" height="7" type="arrow-down"/>
                            </span>
                          </a>
                          <div class="advanced-menu-dropdown">
                            <div class="color-option">
                              ...
                              <div class="color-form">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationErrorColor') }}
                                </p>
                                <div class="d-flex">
                                  <span
                              ...
                              <div class="color-form">
                                <p class="sub-title">
                                  {{ $t('channels.settingsColorCustomizationSuccessColor') }}
                                </p>
                                <div class="d-flex">
                                  <span
                    ...
                    <div class="main" :style="customStyle">
                      <div class="header row">
                        <div class="col-6 offset-3">
                          <img v-if="property.logo && colorCollection.hotel_logo_show"
                               :src="property.logo" alt="logo" class="logo">
                          <img v-else-if="colorCollection.property_logo_show"
                        ...
                        <div class="col-3 d-flex">
                          <b-dropdown v-if="colorCollection.language_block_show"
                            ...
                            <b-dropdown-item v-for="({ id, text }) in propertyLangList" :key="`lang-${id}`"
                          ...
                          <div class="row">
                            <div class="col-xl-5 col-lg-12">
                              <div class="rooms-details-img">
                                ...
                                <a class="text-xs" href="#">5 {{ $t('channels.settingsPictures') }}</a>
                              </div>
                            </div>
                          ...
                          <div class="row">
                            <div class="col-xl-5 col-lg-9 rate-details">
                              <h5 class="meal-included">
                                ...
                                {{ $t('channels.settingsFullIncluded') }}
                              </h5>
                              ...
                              <span class="rate-policy">
                                <span :style="fontStyle(colorCollection.product_name_font_style)">
                                  {{ $t('channels.settingsRateDetails') }}
                                </span>
                                &nbsp;|&nbsp;
                                <span :style="fontStyle(colorCollection.product_name_font_style)">
                                  {{ $t('channels.settingsCancellationPolicy') }}
                                </span>
                              </span>
                            </div>
                            <div class="col-xl-3 col-lg-4 persons">
                              <h5>{{ $t('channels.settingsPersons') }}</h5>
                              <div class="row">
                                <div class="col-6">
                                  <drop-down
                                    id="main-background-type"
                                ...
                                <div class="col-6 adults">
                                  <p>{{ $t('channels.settingsAdults') }}</p>
                                </div>
                              </div>
                              <p class="person-limit">{{ $t('channels.settingsMaxPersons') }}</p>
                            </div>
                            <div class="col-xl-3 col-lg-8 price-room">
                              <h5>{{ $t('channels.settingsPricePerRoom') }}</h5>
                            ...
                            <div class="col-xl-1 col-lg-3 rooms-amount">
                              <h5>{{ $t('channels.settingsRooms') }}</h5>
                              <drop-down
                                id="main-background-type"
                                :disabled="updatePending"
                      ...
                      <div class="footer">
                        <div class="row text-sm">
                          <div class="col-xl-4 col-lg-12 d-flex privacy-policy">
                            {{ $t('channels.settingsLegalNotice') }}&nbsp;|&nbsp;
                            {{ $t('channels.settingsPrivacyPolicy') }}
                          </div>
                          <div class="address col-xl-4 col-lg-12 text-center"
                               v-show="colorCollection.address_show">
                            <p>{{ propertyData.name }}</p>
                            <p>{{ propertyData.street }}, {{ propertyData.zip }} {{ propertyData.city }},
                              {{ countryName }}</p>
                            <p>{{ $t('channels.settingsTelephone') }}: {{ hotelData.tel }}</p>
                            <p>{{ $t('channels.settingsEmail') }}: {{ hotelData.email }}</p>
            ...
            <div class="panel position-relative panel-content my-40">
              <div class="list left-edge">
                <p class="head-line" :class="{ opened: languageOpened }" @click="languageOpened=!languageOpened">
                  ...
                  {{ $t('channels.settingsLanguage') }}
                  <icon width="12" height="7" stroke-width="2" class="icon-arrow" type="arrow-down"/>
                </p>
                <div>
                  <div class="default-language">
                    <p class="text-sm mb-2">{{ $t('channels.settingsDefaultLanguage') }}</p>
                    <drop-down
                      id="default-language"
                      :disabled="updatePending"
                  ...
                  <div class="supported-languages">
                    <p class="text-sm mb-2 title">{{ $t('channels.settingsSupportedLanguages') }}</p>
            ...
            <div class="panel position-relative panel-content my-40">
              <div class="list left-edge">
                <p class="head-line" :class="{ opened: featuresOpened }" @click="featuresOpened=!featuresOpened">
                  ...
                  {{ $t('channels.settingsAdditionalFeaturesTitle') }}
                  <icon width="12" height="7" stroke-width="2" class="icon-arrow" type="arrow-down"/>
                </p>
                <div>
                  <div class="additional-features">
                    <div v-for="(name, option) in $t('channels.settings.additional-features.features')"
                         :key="option" class="feature">
            ...
            <div class="cell-buttons">
              <button type="button" class="btn btn-outline-primary" @click="resetColorCollection"
                      :disabled="updatePending">
                {{ $t('channels.settingsBtnCancel') }}
              </button>
              <button type="button" class="btn btn-primary" @click="updateColor"
                      :disabled="updatePending">
                {{ $t('channels.settingsBtnDone') }}
              </button>
            </div>
          </div>
        </template>
            ...
            <div class="panel position-relative panel-content my-40">
              <div class="list left-edge">
                <p class="head-line" :class="{ opened: emailOpened }" @click="emailOpened=!emailOpened">
                  {{ $t('channels.communicationGuestEmail') }}
                  <icon width="12" height="7" stroke-width="2" class="icon-arrow" type="arrow-down"/>
                </p>
                <div>
                  <hr class="hrGroup">
                  ...
                  <div class="guest-email" v-for="code in langCodes" :key="code" v-show="lang === code">
                    <div class="d-flex justify-content-between">
                      <div class="logo-area">
                        <p class="text-sm">{{ $t('channels.communicationLogoArea') }}</p>
                        <p class="ft-10">{{ $t('channels.communicationLogoSize') }}</p>
                      </div>
                      <div class="date">
                    ...
                    <div class="reservation">
                      <p class="text-md title">{{ $t('channels.communicationReservationTitle') }}</p>
                      <div class="reservation-data">
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationBookingNumber') }}</span>
                          <span>60114-15094050-15209180</span>
                        </div>
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationCheckIn') }}</span>
                          <span>19 Jan 2019</span>
                        </div>
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationCheckOut') }}</span>
                          <span>25 Jan 2019</span>
                        </div>
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationNights') }}</span>
                          <span>2</span>
                        </div>
                        <div class="data flex-column">
                          <div class="room">
                            <span class="title">{{ $t('channels.communicationReservationRooms') }}</span>
                            <span class="title">{{ $t('channels.communicationReservationRoomPrice') }}</span>
                          </div>
                          <div class="room">
                        ...
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationTotalRooms') }}</span>
                          <span>7</span>
                        </div>
                        <div class="data">
                          <span class="title">{{ $t('channels.communicationReservationTotalPersons') }}</span>
                          <span>13</span>
                        </div>
                      </div>
                    </div>
                    <div class="cancellation">
                      <p class="text-md title">{{ $t('channels.communicationCancellationTitle') }}</p>
                      <div class="cancellation-desc">
                        <span>{{ $t('channels.communicationCancellationDescription') }}</span>
                      </div>
                      <div class="price">
                        <p>EUR 420.00</p>
                        <span>{{ $t('channels.communicationCancellationTotalPrice') }}</span>
                      </div>
                      <ValidatedField type="richtext" name="body" :disabled="updatePending" no-icon
                                      :placeholder="$t('channels.communicationBodyPlaceholder')"
                      ...
                      <div class="cancellation-info">
                        <p class="title text-md">
                          {{ $t('channels.communicationCancellationInfoTitle') }}
                        </p>
                        <hr class="hrGroup">
                        <div class="info">
                          <icon width="10" height="10" type="diamond"/>
                          <p class="text-xs">{{ $t('channels.communicationCancellationToCancel') }}</p>
                        </div>
                        <div class="info">
                          <icon width="10" height="10" type="diamond"/>
                          <p class="text-xs">
                            {{ $t('channels.communicationCancellationEnterBooking') }}
                          </p>
                        </div>
                        <b-btn variant="primary" class="btn-cancel-booking" size="sm">
                          {{ $t('channels.communicationCancellationBtnCancel') }}
                        </b-btn>
                        <div class="cancel-desc">
                          <icon width="20" height="20" type="info"/>
                          <span>{{ $t('channels.communicationCancellationCancelBooking') }}</span>
                        </div>
                        <ValidatedField type="richtext" name="signature" :disabled="updatePending" no-icon
                                        :placeholder="$t('channels.communicationSignaturePlaceholder')"
          ...
          <div class="product-block">
              ...
              <div class="row">
                <div class="col-md-6 center-align">
                  <div class="product-top-section--typography">
                    <h1>{{ $t('channels.productTopHeader') }}</h1>
                    <p>{{ $t('channels.productTopDescription') }}</p>
                    <button :disabled="channelIsActive || pending || cstateUpdatePending"
                            @click="activateChannel()"
                            class="btn btn-secondary">{{ $t('channels.productTopBtnActivate') }}</button>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-md-7">
                  <div class="product-top-section--info">
                    <h4>{{ $t('channels.productTopInfo') }}</h4>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="row">
                <div class="col-md-10">
                  <div class="product-features-section--item">
                    ...
                    <p v-html="$t('channels.productFeaturesCalendar')"></p>
                  </div>
                </div>
                <div class="col-md-10">
                  <div class="product-features-section--item">
                    <icon width="100" height="100" type="note-circle-fill"/>
                    <p>{{ $t('channels.productFeaturesBooking') }}</p>
                  </div>
                </div>
                <div class="col-md-10">
                  <div class="product-features-section--item">
                    <icon width="100" height="100" type="code-circle-fill"/>
                    <p>{{ $t('channels.productFeaturesCommission') }}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="row">
                <div class="col-md-12">
                  <div class="product-try-section-head--text">
                    <h4>{{ $t('channels.productTryTitle') }}</h4>
                    <p v-html="$t('channels.productTryDescription')"></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="product-try-section product-section">
              <div class="row">
                <div class="col-md-4 cell-date-picker">
                  <label class="text-xs mb-1" for="check-in">{{ $t('channels.productTryCheckIn') }}</label>
                  <date-picker v-model="checkIn" :min-date="today" id="check-in" />
                </div>
                <div class="col-md-4 cell-date-picker">
                  <label class="text-xs mb-1" for="check-out">{{ $t('channels.productTryCheckOut') }}</label>
                ...
                <div class="col-md-4 cell-buttons">
                  <b-button class="w-100" variant="secondary">
                    {{ $t('channels.productTrySearch') }}
                  </b-button>
                </div>
                <div class="col-12 promo-code">
                  <icon width="18" height="18" type="gift"/>
                  <a href="#">{{ $t('channels.productTryPromoCode') }}</a>
                </div>
              </div>
            </div>
            <div class="product-details-section top product-section">
              <div class="row product-details-section--item">
                ...
                <div class="col-md-5 center-align detail-text">
                  <div>
                    <h4>{{ $t('channels.productDetailsResponsiveTitle') }}</h4>
                    <p v-html="$t('channels.productDetailsResponsiveDescriptionTop')"></p>
                    <p>{{ $t('channels.productDetailsResponsiveDescriptionBottom') }}</p>
                  </div>
                </div>
              </div>
            </div>
            <div class="product-details-section product-section">
              <div class="row flex-md-row flex-column-reverse product-details-section--item">
                <div class="col-md-6 center-align detail-text">
                  <div>
                    <h4>{{ $t('channels.productDetailsBookingTitle') }}</h4>
                    <p>{{ $t('channels.productDetailsBookingDescription') }}</p>
                  </div>
                </div>
                <div class="col-md-6 center-align">
            ...
            <div class="product-details-section bottom product-section">
              <div class="row product-details-section--item">
                ...
                <div class="col-md-6 center-align detail-text">
                  <div>
                    <h4>{{ $t('channels.productDetailsConversionTitle') }}</h4>
                    <p>{{ $t('channels.productDetailsConversionDescriptionTop') }}</p>
                    <p>{{ $t('channels.productDetailsConversionDescriptionBottom') }}</p>
                  </div>
                </div>
              </div>
            </div>
            <div class="product-details-section end product-section">
              <div class="row flex-md-row flex-column-reverse product-details-section--item">
                <div class="col-md-6 center-align detail-text">
                  <div>
                    <h4>{{ $t('channels.productDetailsIntegrationTitle') }}</h4>
                    <p v-html="$t('channels.productDetailsIntegrationDescriptionTop')"></p>
                    <p>{{ $t('channels.productDetailsIntegrationDescriptionBottom') }}</p>
                  </div>
                </div>
                <div class="col-md-6 center-align">
              ...
              <div class="row">
                <div class="col-12">
                  <div class="product-footer-section--head">
                    <h4>{{ $t('channels.productFooterTitle') }}</h4>
                  </div>
                </div>
                <div class="col-12">
                  <div class="product-footer-section--actions">
                    <button :disabled="channelIsActive || pending || cstateUpdatePending"  @click="activateChannel()"
                            class="btn btn-secondary">{{ $t('channels.productFooterBtnActivate') }}</button>
                    <button  @click="askAQuestion()"
                             class="btn btn-outline-light">{{ $t('channels.productFooterBtnAsk') }}</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <b-alert v-if="channelIsInvalid && !updatePending" variant="danger" show>
        ...
        <p class="mb-0">{{ $t('channels.connectErrorInvalidPropertyData') }}</p>
      </b-alert>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters('channel', ['loaded', 'channel', 'ctypes', 'cplans', 'mapped', 'rooms', 'plans', 'settingsPropertyId', 'colors', 'email']),
      ...
      ...mapGetters('user', ['propertyID', 'currency', 'engineURL', 'property']),

      langCodes: () => langCodes,
      tabs() {
        if (this.channel.type === 'push') {
          const tabs = [
            { id: 'connected', title: this.$t('channels.connectTabConnected') },
          ];
          if (this.pendingCTypes.length === 0) {
            tabs.push(
              { id: 'pending', title: this.$t('channels.connectTabPending') },
            );
          } else {
            tabs.unshift(
              { id: 'pending', title: this.$t('channels.connectTabPending') },
        ...
        const tabs = [
          { id: 'mapping', title: this.$t('channels.headersMapping') },
        ];
        if (this.channel.default) {
          tabs.push(
            { id: 'urls', title: this.$t('channels.tabsUrls') },
            { id: 'promos', title: this.$t('channels.tabsPromos') },
            { id: 'settings', title: this.$t('channels.tabsSettings') },
            { id: 'communication', title: this.$t('channels.tabsCommunication') },
            { id: 'product', title: this.$t('channels.tabsProduct') },
          );
        }
        return tabs;
      },
      btnColorTabs() {
        const tabs = [
          { id: 'normal', title: this.$t('channels.settingsNormal') },
          { id: 'mouse', title: this.$t('channels.settingsMouse') },
          { id: 'text', title: this.$t('channels.settingsText') },
        ];
        return tabs;
      },
      mainColorTabs() {
        const tabs = [
          { id: 'main', title: this.$t('channels.settingsMain') },
          { id: 'text', title: this.$t('channels.settingsText') },
        ];
        return tabs;
      },
      textColorTabs() {
        const tabs = [
          { id: 'text', title: this.$t('channels.settings.text') },
        ];
        return tabs;
      },
      channelName() {
      ...
      periodValue() {
        ...
        return this.$tc(`channels.connectTimeUnitsValues${period.unit}`, period.number);
      },
      endDateValue() {
        const { period } = this.channel;
        return moment(period.until).format(this.$t('channels.connectTransmissionFormat'));
      },
      fieldsValues() {
      ...
      urlCustom() {
        ...
        return `${this.engineURL}&hotelcode=${this.propertyID}${pc}${rc}`;
      },
      promoCreatePlans() {
      ...
      promoModalTitle() {
        ...
        if (id == null) {
          return this.$t(`channels.promoModalTitleAdd${mode}`);
        }
        if (outdated) {
          return this.$t(`channels.promoModalTitleView${mode}`, { code });
        }
        return this.$t(`channels.promoModalTitleEdit${mode}`, { code });
      },
      promoOkTitle() {
      ...
      typeList() {
        const types = [];
        const backgroundType = ['flat', 'gradient'];
        Object.keys(backgroundType).forEach((key) => {
          types.push({ id: key, text: this.$t(`channels.settingsColorCustomizationBackgroundType${key}`) });
        });
        return types;
      },
      availableAmount() {
      ...
      countryName() {
        ...
        Object.keys(this.countries).forEach((key) => {
          if (this.countries[key].code === this.propertyData.country) {
      ...
      locationUrl() {
        return `https://www.google.com/maps/place/${this.propertyData.latitude},${this.propertyData.longitude}`;
      },
      channelIsActive() {
      ...
      filteredRows() {
        ...
        if (!filter) return this.pendingCTypes;
        return this.pendingCTypes
          .filter(({ typeid, name }) => (`${typeid}`.includes(filter) || name.toLowerCase().includes(filter)));
      },
      mappedPlansSearch() {
        const filter = this.filter.trim().toLowerCase();
        if (!filter) return this.mappedPlans;
        return this.mappedPlans
          .filter(({ ctype }) => (ctype.toLowerCase().includes(filter)));
      },
    },
    methods: {
      ...mapActions('channel', [
        ...
        'createContract', 'updateContract', 'deleteContract', 'fetchProperty', 'fetchLanguages',
        'fetchColor', 'saveColor', 'fetchEmail', 'saveEmail',
      ]),
      ...mapActions('user', ['getProperty']),
      ...
      async setChannelId(id) {
        ...
        try {
          ...
          await Promise.allSettled([
            this.fetchColor(this.settingsPropertyId),
            this.fetchEmail(this.settingsPropertyId),
          ]);
        } catch (e) {
      ...
      freshData() {
        ...
        Object.keys(this.emailCollection).forEach((code) => {
          ...
          const signature = `${this.$t('channels.communicationBestRegards')}<br>${this.hotelData.name},<br>${this.hotelData.street}<br>${this.hotelData.city}, ${this.hotelData.zip}<br>${this.countryName}.<br>${this.$t('addr.phone')}: ${this.hotelData.tel}`;
      ...
      async updateRatePlans({ id, values }) {
        try {
          ...
          this.$toastr.s({
            title: this.$t('channels.alertSaved'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
      ...
      async connectPlans() {
        try {
          ...
          this.cmap.selected = [];
          this.$toastr.s({
            title: this.$t('channels.connectAlertConnected'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
      ...
      async disconnectPlan(item) {
        try {
          ...
          this.$toastr.s({
            title: this.$t('channels.connectAlertDisconnected'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
      ...
      async updateConnection(item, updates) {
        try {
          ...
          this.$toastr.s({
            title: this.$t('channels.connectAlertUpdated'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
      ...
      async activateChannel() {
        if (this.channel.status === 'inactive') {
          ...
          this.$toastr.s({
            title: this.$t('channels.connectAlertActivated'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
      ...
      formatTime(date) {
        const fdate = this.$t('channels.communicationDateFormat');
