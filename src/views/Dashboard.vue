<template>
  <div>
    ...
    <div class="page-dashboard" v-if="!dashboardDataPending">
      <div class="panel">
        <div class="panel-dropdown-item d-flex justify-content-start align-items-center">
        ...
        <div class="d-flex pb-4 justify-content-center justify-content-md-end">
          <b-btn variant="primary" pill class="download-btn" v-if="report != null"
                 target="_blank" :href="pdfUrl" download
                 :disabled="report == null">
            {{ $t('dashboard.buttonDownload') }}
      ...
      <b-modal no-close-on-backdrop ref="setting-modal" hide-header hide-footer :title="$t('dashboard.settingTitle')">
        <WidgetStatus
          @hide="hide"
  ...
  export default {
    ...
    computed: {
      ...
      timePeriodKeys() {
        ...
        timePeriodDashboardKeys.forEach((key) => {
          ...
          const splitStr = `${key}`.toLowerCase().split('-').join('');
          const newTimePeriod = splitStr.toLowerCase().split('_').pop();
          periodKeys.push({
            key,
            time: this.$t(`dashboard.timePeriodTypes${newTimePeriod}`),
          });
        });
    ...
    methods: {
      ...
      initDropdownMenuValues() {
        ...
        this.timePeriod = {
          title: this.$t('dashboard.timePeriodLabel'),
      ...
      updateApiData() {
        ...
        const bookingFields = [
          {
            label: this.$t('dashboard.channelTitle'),
            key: 'channel',
            sortable: false,
          },
          {
            label: this.$t('dashboard.grossBookings'),
            key: 'gross_bookings',
            sortable: true,
          },
          {
            label: this.$t('dashboard.cancellations'),
            key: 'cancellations',
            sortable: true,
          },
          {
            label: this.$t('dashboard.netBookings'),
            key: 'net_bookings',
            sortable: true,
          },
          {
            label: this.$t('dashboard.netBookingsP'),
            key: 'net_bookings_p',
            sortable: true,
          },
        ];
        ...
        const bookingVolumeTableFields = [
          ...
          {
            label: this.$t('dashboard.grossBookingsVolume'),
            key: 'gross_bookings_volume',
            sortable: true,
          },
          {
            label: this.$t('dashboard.cancellationsVolume'),
            key: 'cancellations_volume',
            sortable: true,
          },
          {
            label: this.$t('dashboard.netBookingsVolume'),
      ...
      updateWidgetValues() {
        ...
        this.booksGroupData.push({
          ...
          content: {
            ...
            dataPoints: [
              ...
              {
                label: this.$t('dashboard.cancellationsBookings'),
                y: (-1) * this.apiData.Cancellations,
                displayValue: this.apiData.Cancellations,
                color: '',
              },
              {
                label: this.$t('dashboard.netBookingValue'),
        ...
        this.booksGroupData.push({
          ...
          content: {
            ...
            dataPoints: [
              {
                label: this.$t('dashboard.grossBookingsRoomnights'),
              ...
              {
                label: this.$t('dashboard.netBookingsRoomnights'),
        ...
        for (let i = 0; i <= 11; i += 1) {
          labels.push(this.$t(`dashboard.monthsSort${i}`));
        }
        id = -1;
