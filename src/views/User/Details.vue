      ...
      <ValidationObserver ref="form" slim v-slot="{ handleSubmit }">
        <b-form @submit.prevent.stop="handleSubmit(submitForm)" novalidate>
          ...
          <template v-else>
            ...
            <b-form-group>
              <radio v-model="skipProperty" :val="true" :disabled="pending" class="mb-3 text-left">
                {{ $t('details.property-register-later') }}
              </radio>
              <radio v-model="skipProperty" :val="false" :disabled="pending" class="mb-1 text-left">
                {{ $t('details.property-register-now') }}
              </radio>
            </b-form-group>

            <template v-if="skipProperty===false">
              <b-form-group class="text-left mb-0">
                <label for="name" class="text-sm mb-1">{{ $t('addr.property-or-property-name') }}</label>
          ...
          <submit-button :loading="pending" :disabled="pdFilled && skipProperty==null">
            {{ $t(`details.steps.${+pdFilled}.button`) }}
          </submit-button>
          <div class="alert alert-danger" role="alert" v-if="propertyRegistrationError != null">
            {{ propertyRegistrationError }}
          </div>
          <p v-if="0&&pdFilled" class="m-0">
            <b-btn variant="link" class="m-0 p-0 h-auto"
                   @click.stop="skipPropertyRegistration">{{ $t('details.steps.1.button-skip') }}</b-btn>
          </p>
        </b-form>
      </ValidationObserver>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      propertyRegistrationError() {
        return this.validationError ? this.validationError.firstErrorFor('property') : null;
      },
      allCurrencies() {
