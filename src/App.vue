  ...
  <fragment>
    <nav class="navbar-wrapper navbar--mobile" :class="{ 'app-loading': !hostLoaded }">
      <div class="navbar d-md-none">
        ...
        <div class="profile">
          <b-dropdown v-if="asLoggedIn && hasProperties" size="sm" toggle-tag="div"
            ...
            <template #button-content>
              ...
              <icon v-if="multipleProperties" stroke-width="2" width="12"
                    height="7" type="arrow-down" class="ml-2 caret"/>
            </template>

            <b-dropdown-group v-if="multipleProperties">
              <b-dropdown-form @submit.stop.prevent>
                <search-filter class="w-100"
                               v-model="propertiesFilter" autofocus :placeholder="$t('placeholder.filter-property')"/>
              </b-dropdown-form>
              <b-dropdown-item v-for="{id, name, city} in filteredProperties" :key="`property-${id}`"
                               class="text-nowrap" @click="switchProperty(id)" :disabled="id === propertyID">
                <span class="text-primary property-id">{{ id }}</span>
                <span class="separator"></span>
                <span>{{ name }}</span><span v-if="city">{{ ', '+city }}</span>
              </b-dropdown-item>
              <b-dropdown-header v-if="!filteredProperties.length">
                {{ $t('properties.filterNoProperties') }}
              </b-dropdown-header>
            </b-dropdown-group>
          </b-dropdown>
        ...
        <div class="content-header d-none d-md-flex align-items-center justify-content-between"
          ...
          <div class="profile">
            <template v-if="hasProperties">
              <span class="text-primary">{{ propertyID }}</span>
              <div class="separator"></div>
              <span v-if="!multipleProperties">{{ propertyName }}</span>
            </template>
            <b-dropdown v-if="multipleProperties" size="sm" toggle-tag="div" toggle-class="profile-menu"
                        menu-class="mt-3 properties-dropdown" variant="link" no-caret right @show="propertiesFilter=''">
              <template #button-content>
                <span>{{ propertyName }}</span>
                <icon class="ml-2" stroke-width="2" width="12" height="7" type="arrow-down"/>
              </template>
              <b-dropdown-form @submit.stop.prevent>
                <search-filter v-model="propertiesFilter" autofocus :placeholder="$t('placeholder.filter-property')" />
              </b-dropdown-form>
              <b-dropdown-item v-for="{id, name, city} in filteredProperties" :key="`property-${id}`"
                               class="text-nowrap" @click="switchProperty(id)" :disabled="id === propertyID">
        ...
        <footer class="d-none" :class="{ 'd-sm-block': !asLoggedIn }">
          <ul class="footer-menu">
            <template v-if="!hideLoginFooterLinks">
              <li><a :href="`https://www.hotelbooking.com/${lang.code}/`"
                     target="_blank">{{ $t('links.home') }}</a></li>
              <li><a :href="`https://www.hotelbooking.com/${lang.code}/#benefits`"
                     target="_blank">{{ $t('links.benefits') }}</a></li>
              <li><a :href="`https://www.hotelbooking.com/${lang.code}/highlights/`"
                     target="_blank">{{ $t('links.highlights') }}</a></li>
              <li><a :href="`https://www.hotelbooking.com/${lang.code}/developers-connect/`"
                     target="_blank">{{ $t('links.devconnect') }}</a></li>
              <li><a :href="`https://www.hotelbooking.com/${lang.code}/price/`"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      ...mapGetters('user', [
        'user', 'hasProperties', 'propertyName', 'propertyID', 'loggedIn', 'sessionExpired', 'lang', 'multipleProperties', 'properties',
      ...
      filteredProperties() {
        const filter = this.propertiesFilter.trim().toLowerCase();
        if (!filter) return this.properties;
        return this.properties.filter(({ id, name, city }) => (`${id}`.includes(filter) || name.toLowerCase().includes(filter) || (city !== '' && city !== null && city.toLowerCase().includes(filter))));
      },
      userName() {
        const { user } = this;
      ...
      legalPagesList() {
        const pages = [
          { id: 6, slug: 'legal-notice', title: 'Legal Notice' },
          { id: 4, slug: 'terms-and-conditions', title: 'Terms & Conditions' },
          { id: 11, slug: 'privacy-policy', title: 'Privacy Policy' },
          { id: 9, slug: 'data-processing-agreement', title: 'Data Processing Agreement' },
          { id: 10, slug: 'data-processors', title: 'Data Processors' },
        ];
    ...
    methods: {
      ...
      logo() {
        this.resetColorScheme();
        if (this.propertyID && this.asLoggedIn) {
          this.user.properties.forEach((t) => {
            if (t.id === this.propertyID) {
              if (t.property_group) {
      ...
      hoverleave() {
        this.hovered = false;
        this.$refs.sidenavdata.toggleMenu();
      },
      resetColorScheme() {
        this.color_schema = {
          r: 74,
