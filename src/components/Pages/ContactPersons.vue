      ...
      <ValidationObserver ref="contactForm" slim>
        <div class="row contactModalRow">
          <div class="col-md-6 col-sm-12">
            <label class="text-xs" for="position">{{ $t('contactPersons.modalPosition') }}</label>
            <ValidatedField
          ...
          <div class="col-md-3 col-sm-6">
            <label class="text-xs" for="salutation">{{ $t('contactPersons.modalSalutation') }}</label>
            <drop-down
              id="salutation"
              id-key="value"
          ...
          <div class="col-md-3 col-sm-6">
            <label class="text-xs" for="language">{{ $t('contactPersons.modalLanguage') }}</label>
            <ValidatedField
              id="language"
              name="language"
              class="mb-0"
          ...
          <div class="col-md-6 col-sm-12">
            <label class="text-xs" for="surname">{{ $t('contactPersons.modalSurname') }}</label>
            <ValidatedField
          ...
          <div class="col-md-6 col-sm-12">
            <label class="text-xs" for="firstname">{{ $t('contactPersons.modalFirstName') }}</label>
            <ValidatedField
          ...
          <div class="col-md-6 col-sm-12">
            <label class="text-xs" for="mail">{{ $t('contactPersons.modalEmailAddress') }}</label>
            <ValidatedField
          ...
          <div class="col-md-6 col-sm-12">
            <label class="text-xs" for="phone">{{ $t('contactPersons.modalPhoneNumber') }}</label>
            <ValidatedField
              group-class=""
              name="phone"
              id="phone"
      ...
      <div class="list d-none d-md-block left-edge">
        <div class="contacts-table" v-if="loaded">
          <table class="w-100">
            <thead>
              <tr>
                <th class="w-position">{{ $t('contactPersons.headersPosition') }}</th>
                <th>{{ $t('contactPersons.headersLanguage') }}</th>
                <th>{{ $t('contactPersons.headersSurname') }}</th>
                <th>{{ $t('contactPersons.headersFirstName') }}</th>
                <th>{{ $t('contactPersons.headersPhoneNumber') }}</th>
                <th>{{ $t('contactPersons.headersEmailAddress') }}</th>
                <th class="w-actions">{{ $t('contactPersons.actions') }}</th>
              </tr>
            </thead>
            <tbody v-if="noContacts">
              <tr>
                <td colspan="8" class="w-empty">{{ $t('contactPersons.noContacts') }}</td>
              </tr>
            </tbody>
        ...
        <div v-if="loaded">
          <b-btn pill variant="outline-primary" class="add-new-contact"
            ...
            {{ $t('contactPersons.buttonAdd') }}
          </b-btn>
        </div>
      </div>
      <div class="d-md-none list-item" v-if="noContacts">
        <div class="w-empty">{{ $t('contactPersons.noContacts') }}</div>
      </div>
      <div class="d-md-none list-item" v-for="row in contacts" :key="row.id">
        <div class="d-flex">
          <div class="c-1">
            <p class="label">{{ $t('contactPersons.headersPosition') }}</p>
            <p>{{ row.position }}</p>
          </div>
          <div class="dots">
            <b-dropdown size="sm" toggle-tag="span" variant="link" no-caret right :disabled="updatePending">
              ...
              <b-dropdown-item @click="onEditContact(row)">{{ $t('contactPersons.buttonEdit') }}</b-dropdown-item>
              <b-dropdown-item @click="onDeleteContact(row.id, true)"
                               :disabled="row.protected">{{ $t('contactPersons.buttonDelete') }}</b-dropdown-item>
            </b-dropdown>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersSalutation') }}</p>
            <p>{{ getSalutation(row.salutation) }}</p>
          </div>
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersLanguage') }}</p>
            <p>{{ $t(`langs.${row.language}`) }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersSurname') }}</p>
            <p class="w-sm-surname" >{{ row.surname }}</p>
          </div>
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersFirstName') }}</p>
            <p>{{ row.firstname }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersPhoneNumber') }}</p>
            <p>{{ row.phone }}</p>
          </div>
          <div class="w-50">
            <p class="label">{{ $t('contactPersons.headersEmailAddress') }}</p>
    ...
    computed: {
      ...
      modalTitle() {
        if (this.contact.id == null) {
          return this.$t('contactPersons.modalTitleAdd');
        }
        return this.$t('contactPersons.modalTitleEdit');
      },
      noContacts() {
