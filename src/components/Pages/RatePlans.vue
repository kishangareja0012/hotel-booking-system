...
<template>
  <div class="component-rateplans">
    ...
    >
      ...
      <ValidationObserver ref="planForm" slim>
        <div class="edge" :class="{ active: edgeRoomLang }">
          <h3 :content="$t('ratePlans.sectionConnect')"></h3>
          ...
          <div class="link-figure">
            ...
            <h5>{{ $t('ratePlans.sectionPlanName') }}</h5>
            <div class="lang-choice" v-for="code in langCodes" :key="code" v-show="lang === code">
              ...
              <ValidatedField type="richtext" class="plan-desc" :placeholder="$t('ratePlans.description')"
                              no-icon :rules="rulesForLang('desc')" :name="`lang-${code}-desc`"
                              v-model="plan.langs[code].desc" :disabled="updatePending"/>
            </div>
            <h5>{{ $t('ratePlans.sectionRoomType') }}</h5>
          </div>
          <drop-down
            id="dd-rooms"
            v-model="plan.room"
        ...
        <div class="edge edge-settings">
          <h4 :class="{ opened: settingsOpened }" @click="settingsOpened=!settingsOpened">
            {{ $t('ratePlans.sectionSettings') }}
            <icon width="12" height="7" stroke-width="2" type="arrow-down"/>
          </h4>
          <div>
            <h3 :content="$t('ratePlans.travelDays')"></h3>
              ...
              <div class="col cell-travel">
                <div class="cell-travel-title">
                  <p v-for="key in travelOpts" :key="`tt-${key}`"
                     v-text="key?$t(`ratePlans.${key}`):''"></p>
                </div>
                <div class="cell-travel-alls">
                  <p v-for="key in travelOpts" :key="`ta-${key}`">
                    <template v-if="!key">{{ $t('ratePlans.daysAll') }}</template>
            ...
            <h3 :content="$t('ratePlans.los')"></h3>
            <div class="row row-settings">
              <div class="col cell-minlos">
                <label class="text-xs" for="minlos">{{ $t('ratePlans.minimum') }}</label>
                <ValidatedField
              ...
              <div class="col cell-maxlos">
                <label class="text-xs" for="maxlos">{{ $t('ratePlans.maximum') }}</label>
                <ValidatedField
            ...
            <h3 :content="$t('ratePlans.mealType')"></h3>
            <div class="row row-settings">
            ...
            <h3 :content="$t('ratePlans.cancelPolicies')"></h3>
            <div class="row row-settings">
              <div class="col cell-cancels">
                <drop-down
                  ...
                  select-all
                  multiple
                  title-all="ratePlans.cancelAll"
                  title="ratePlans.cancelSome"
            ...
            <h3 :content="$t('ratePlans.paymentPolicy')"></h3>
            <div class="row row-settings">
            ...
            <h3 :content="$t('ratePlans.sectionBookable')"></h3>
            <div class="row row-settings">
                    ...
                    <i18n :tag="false" path="ratePlans.periodsDates">
                      ...
                      <template #to>
                        <date-picker
                          ...
                          :disabled="updatePending || plan.bookable.mode !== bookable.periods"
                          :placeholder="$t('ratePlans.placeholderEndDate')"
            ...
            <div class="row row-settings bookable-settings">
              <div class="col bookable-wrapper">
                ...
                <i18n path="ratePlans.fromto" :tag="false">
                  <template #from>
                    <AmountPercent
                      simple :symbol="$tc('ratePlans.placeholderDays', plan.bookable.from)"
                  ...
                  <template #to>
                    <AmountPercent
                      simple :symbol="$tc('ratePlans.placeholderDays', plan.bookable.to)"
              ...
              <div class="row row-settings bookable-settings">
                <div class="col bookable-wrapper">
                  ...
                  <i18n path="ratePlans.until" :tag="false">
                    <template #days>
                      <AmountPercent
                        simple :symbol="$tc('ratePlans.placeholderDays', plan.bookable.until)"
              ...
              <div class="row row-settings">
                <div class="col bookable-wrapper">
                  ...
                  <i18n path="ratePlans.within" :tag="false">
                    <template #days>
                      <AmountPercent
                        simple :symbol="$tc('ratePlans.placeholderDays', plan.bookable.within)"
        ...
        <div class="edge edge-price">
          ...
          <div>
            ...
            <div class="row row-edit">
              <div class="col cell-edit-label double">
                <radio v-model="plan.price.mode" :disabled="disabledForPromo(true) || updatePending || !roomSelected"
                       name="price_mode"
                       val="standard">{{ $t('ratePlans.standardPrice') }}</radio>
              </div>
              <div class="col cell-edit-tip">
                {{ $t('ratePlans.standardPriceTip') }}
              </div>
            </div>
            <div class="row row-edit">
              <div class="col cell-edit-label smaller">
                <radio v-model="plan.price.mode" :disabled="disabledForPromo(true) || updatePending || !roomSelected"
                       name="price_mode"
                       val="fixed" class="text-nowrap">{{ $t('ratePlans.fixedPrice') }}</radio>
              </div>
              ...
              <div class="col cell-edit-tip">
                {{ $t('ratePlans.fixedPriceTip') }}
              </div>
            </div>
            <h3 class="stdcalc-title" :content="$t('ratePlans.standardPriceMore')"></h3>
            ...
            <h3 :content="$t('ratePlans.occupancyMore')"></h3>
            <div class="row row-edit row-occupancy no-tip">
              <div class="col cell-edit-field single">
                <label class="text-xs" for="occ-min">{{ $t('ratePlans.minimum') }}</label>
                <ValidatedField
              ...
              <div class="col cell-edit-field single inverted">
                <label class="text-xs" for="occ-std">{{ $t('ratePlans.standard') }}</label>
                <ValidatedField
                  type="number" integer purenumber no-icon no-tooltip
              ...
              <div class="col cell-edit-field single">
                <label class="text-xs" for="occ-max">{{ $t('ratePlans.maximum') }}</label>
                <ValidatedField
                  type="number" integer purenumber no-icon no-tooltip
            ...
            <div class="inner" v-if="notStandardOccupancy">
              ...
              <h3 class="line" :content="$t('ratePlans.childrenBed')"></h3>
              <div class="row row-edit no-tip" v-for="cs in plan.price.calc.children" :key="`cbed-${cs.id}`">
                <div class="col cell-edit-field single smaller">
                  <ValidatedField type="number" purenumber integer min="1" max="18" no-icon no-tooltip
                                  :placeholder="$t('ratePlans.age')" :rules="rulesFor('child-age')"
        ...
        <div class="edge">
          <h3 :content="$t('date.validityPeriod')"></h3>
            ...
            <div class="row row-blockout" v-for="(blockout, idx) in plan.blockouts" :key="`bl-${idx}`">
              ...
              <div class="col-12 cell-block-until">
                ...
                <date-picker :id="`date-block-until-${idx}`" v-model="blockout.until" :min-date="today"
                             ...
                             position="left-md-right"
                             :disabled="updatePending"
                             @input="blockUntilChanged($event, idx)" :format="dateFormat" />
              </div>
    ...
    <RatePlansDetail id="planDetailModal"
    ...
    <div class="panel position-relative panel-content">
      <div class="list d-none d-md-block left-edge">
        <div class="d-none d-md-flex justify-content-between">
          <p class="head-line">
            <span>{{ $t('ratePlans.heading') }}</span><spinner v-if="!loaded" />
          </p>
          <search-filter v-model="filter" :placeholder="$t('ratePlans.filterTip')"/>
        </div>
        <div class="plans-table" v-if="loaded">
          <search-filter
            ...
            autofocus
            :placeholder="$t('ratePlans.search')"
            :disabled="updatePending"
          />
          <table class="w-100">
            <thead>
              <tr>
                ...
                <th class="w-cancels" :class="active === 'policy' ? 'active' : ''" @click="sortData('policy');">
                  <div>
                    {{ $t('ratePlans.headersCancelPolicy') }}
                    <div>
                      <icon width="12" height="12" type="arrow-up"/>
                      <icon width="12" height="12" type="arrow-down"/>
                    </div>
            ...
            <tbody v-if="noPlans">
              <tr>
                <td colspan="6" class="w-empty">{{ $t('ratePlans.noPlans') }}</td>
              </tr>
            </tbody>
        ...
        <div v-if="loaded">
          <b-btn pill variant="outline-primary" class="add-new-plan"
            ...
            <icon width="10" height="11" type="plus"/>
            {{ $t('ratePlans.buttonAdd') }}
          </b-btn>
        </div>
      </div>

      <p class="head-line d-md-none"><span>{{ $t('ratePlans.heading') }}</span><spinner v-if="!loaded" /></p>
      <div class="d-md-none list-item" v-if="noPlans">
        <div class="w-empty">{{ $t('ratePlans.noPlans') }}</div>
      </div>
      <div class="d-md-none list-item" v-for="row in rateplans" :key="row.id">
        <div class="d-flex">
          <div class="c-1">
            <p class="label">{{ $t('ratePlans.headersName') }}</p>
            <p class="text-break">
              {{ row.langs.en.name }}
              <b-badge v-if="row.promo" class="ml-1"
                       :variant="row.promomode === 'promo' ? 'primary' : 'success'">{{ row.promo }}</b-badge>
        ...
        <div class="d-flex line">
          ...
          <div class="w-50">
            <p class="label">{{ $t('ratePlans.headersMinStay') }}</p>
            <p>{{ row.minlos }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-50">
            <p class="label">{{ $t('ratePlans.headersMealPlan') }}</p>
            <p>{{ mealNameById(row.meals) }}</p>
          </div>
          <div class="w-50">
            <p class="label">{{ $t('ratePlans.headersLinkedRoom') }}</p>
            <p>{{ roomNameById(row.room) }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="c-1">
            <p class="label">{{ $t('ratePlans.headersCancelPolicy') }}</p>
            <li class="cancel-policy-display-text"
                :class="{empty: row.cancels.length === 0}"
...
<script>
  ...
  import RatePlansDetail from '@/components/Pages/RatePlansDetail.vue';

  const travelOpts = ['', 'bdays', 'adays', 'ddays'];

  export default {
    name: 'RatePlans',
    components: { RatePlansDetail },
    data: () => ({
      plan: {},
      lang: 'en',
      detailLangs: {},
    ...
    computed: {
      ...
      modalTitle() {
        if (this.plan.id == null) {
          return this.$t('ratePlans.titleAdd');
        }
        const { id, langs: { en: { name } } } = this.plan;
        return this.$t('ratePlans.titleEdit', { id, name });
      },
      planFinalPrice() {
      ...
      policiesLocalized() {
        return [
          {
            id: '0',
            ...this.$t('ratePlans.noPaymentPolicy'),
          },
          ...this.policies,
        ];
      },
      ...
      priceHeader() {
        if (!this.roomSelected) return this.$t('ratePlans.price');
        const occupancy = this.$tc('ratePlans.priceOccupancy', this.plan.occupancy.std);
        return this.$t('ratePlans.priceFor', { occupancy });
      },
      rateplans() {
        let { plans } = this;
