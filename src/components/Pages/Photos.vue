  ...
  <div class="component-neogallery">
    <div class="panel">
      <div class="position-relative panel-title title" :class="titleClass">
        ...
        <b-btn variant="outline-primary" v-b-modal.mediaLibraryModal
               @click="resetLibraryModal">{{ $t('photos.buttonLibrary') }}</b-btn>
      </div>
    ...
    >
      ...
      <template #modal-title>
        <div class="">
          <div>{{ $t('photos.libraryTitle') }}</div>
          <div class="filter-option">
            <div class="resolution-filter">
              <check-box v-model="lowresOnly">{{ $t('photos.lowresOnly') }}</check-box>
            </div>
            <search-filter v-model="searchQuery" :placeholder="$t('photos.search')" noborder />
          </div>
        </div>
      </template>
      <ValidationObserver>
          ...
          <div class="media-detail" :class="{ active: details.id != null }">
            ...
            <div class="detail-decription" v-if="details.id!=null">
              <div class="detail-info">
                <div class="name-info">
                  <div class="real-name">
                    <div>{{ $t('photos.name') }}</div>
                    <div class="room-name real">
                      {{ details.name }}
                    </div>
                  </div>
                  <div class="display-name">
                    <div>{{ $t('photos.displayName') }}</div>
            ...
            <div class="detail-empty" v-else>{{ $t('photos.noImageSelected') }}</div>
          </div>
        </div>
      </ValidationObserver>
    </b-modal>
