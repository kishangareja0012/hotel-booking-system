  ...
  <div class="component-mealplans">
    ...
    >
      ...
      <ValidationObserver ref="mealForm" slim>
        <div class="edge" :class="{ active: edgeMealLang }">
          <h3 :content="$t('mealPlans.modalSectionMealName')"></h3>
          ...
          <div class="link-figure">
            ...
            <div class="lang-choice" v-for="code in langCodes" :key="code" v-show="lang === code">
              ...
              <b-textarea no-resize class="plan-desc" :placeholder="$t('mealPlans.modalDescription')"
                          no-icon :rules="rulesForLang(code, 'desc')" :name="`lang-${code}-desc`"
                          v-model="meal.langs[code].desc" :disabled="updatePending"/>
            </div>
            <h5>{{ $t('mealPlans.modalSectionMealType') }}</h5>
          </div>
          <drop-down
            id="dd-typecodes"
        ...
        <div class="edge edge-amount" :class="{ active: edgeMealAmount }">
          <h3 :content="$t('mealPlans.modalSectionBasicRate')"></h3>
                ...
                <div class="col cell-travel">
                  <div class="cell-travel-title">
                    <p v-for="key in travelOpts" :key="`tt-${key}`"
                       v-text="key?$t(`mealPlans.modalTravelOptsDays`):''"></p>
                  </div>
                  <div class="cell-travel-alls">
                    <p v-for="key in travelOpts" :key="`ta-${key}`">
                      <template v-if="!key">{{ $t('mealPlans.modalDaysAll') }}</template>
                  ...
                  <div class="cell-travel-wd" v-for="(wd, idxx) in weekdays" :key="`twd-${wd}`">
                    <p v-for="key in travelOpts" :key="`td-${key}`">
                      <template v-if="!key">{{ $t('mealPlans.day'+`${idxx}`) }}</template>
                      <check-box v-else empty :disabled="updatePending"
          ...
          <div class="danger">
            <div class="row row-blockout" v-for="(blockout, idx) in meal.blockouts" :key="`bl-${idx}`">
              <div class="row row-travel d-none d-md-flex">
                <div class="col cell-travel">
                  ...
                  <div class="cell-travel-wd" v-for="(wd, idx) in weekdays" :key="`twd-${wd}`">
                    <p v-for="key in travelOpts" :key="`td-${key}`">
                      <template v-if="!key">{{ $t('mealPlans.day'+`${idx}`) }}</template>
                      <check-box v-else empty :disabled="updatePending"
              ...
              <div class="col-12 cell-block-until">
                <label class="text-xs" :for="`date-block-until-${idx}`">{{ $t("date.until") }}</label>
                <date-picker
                  :id="`date-block-until-${idx}`"
                  v-model="blockout.until"
                  :min-date="today"
                  grow="up"
                  ref="blockUntil"
                  position="left-md-right"
    ...
    <div class="panel position-relative panel-content">
      <div class="list d-none d-md-block left-edge">
        <p class="head-line"><span>{{ $t('mealPlans.heading') }}</span><spinner v-if="!loaded" /></p>
          ...
          <table class="w-100">
            <thead>
              <tr>
                ...
                <th class="w-name">
                  {{ $t('mealPlans.headersType') }}
                </th>
                <th class="w-until">
                  {{ $t('mealPlans.headersValiduntil') }}
                </th>
            ...
            <tbody v-if="noMeals">
              <tr>
                <td colspan="6" class="w-empty">{{ $t('mealPlans.noMeals') }}</td>
              </tr>
            </tbody>
              ...
              <tr>
                ...
                <td class="actions">
                  ...
                  <b-btn
                    ...
                    @click="processDeleteMeal(row.id)"
        ...
        <div v-if="loaded">
          <b-btn pill variant="outline-primary" class="add-new-plan"
            ...
            <icon width="10" height="11" type="plus"/>
            {{ $t('mealPlans.buttonAdd') }}
          </b-btn>
        </div>
      </div>

      <p class="head-line d-md-none"><span>{{ $t('mealPlans.heading') }}</span><spinner v-if="!loaded" /></p>
      <div class="d-md-none list-item" v-if="noMeals">
        <div class="w-empty">{{ $t('mealPlans.noMeals') }}</div>
      </div>
      <div class="d-md-none list-item" v-for="row in mealplans" :key="row.id">
        <div class="d-flex">
          <div class="c-1">
            <p class="label">{{ $t('mealPlans.headersName') }}</p>
          ...
          <div class="dots">
            <b-dropdown size="sm" toggle-tag="span" variant="link" no-caret right :disabled="updatePending">
              ...
              <b-dropdown-item @click="processDeleteMeal(row.id)" :disabled="row.protected || row.deletable">{{
                $t("buttons.delete")
              }}</b-dropdown-item>
            </b-dropdown>
          </div>
        ...
        <div class="d-flex line">
          ...
          <div class="w-50">
            <p class="label">{{ $t('mealPlans.headersType') }}</p>
            <p>{{ mealtype(row.typecode) }}</p>
          </div>
        </div>
      </div>
    ...
    computed: {
      ...
      modalTitle() {
        if (this.meal.id == null) {
          return this.$t('mealPlans.modalTitleAdd');
        }
        const { id, langs: { en: { name } } } = this.meal;
        return this.$t('mealPlans.modalTitleEdit', { id, name });
      },
    },
    methods: {
      ...
      showSuccessMessage(type) {
        const title = type === 'saved'
          ? this.$t('mealPlans.alertSaved')
          : this.$t('mealPlans.alertDeleted');
        this.$toastr.s({
          title,
          msg: '',
          timeout: 3000,
