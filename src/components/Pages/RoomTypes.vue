    ...
    >
      ...
      <ValidationObserver ref="roomForm" slim>
        <div class="edge" :class="{ active: edgeRoomLang }">
          <h3 :content="$t('roomTypes.sectionRoomName')"></h3>
        ...
        <div class="edge edge-amount" :class="{ active: edgeRoomAmount }">
          <h3 :content="$t('roomTypes.sectionAmount')"></h3>
        ...
        <div class="edge edge-occupancy" :class="{ active: edgeRoomOccStd }">
          ...
          <div>
            <div class="row row-occupancy">
              ...
              <div class="col cell-edit-field">
                <label class="text-xs" for="occ-max">{{ $t('roomTypes.maxOccupancy') }}</label>
                <div class="with-guests">
                  <ValidatedField
    ...
    <div class="panel position-relative panel-content">
      <div class="list d-none d-md-block left-edge">
        <div class="d-none d-md-flex justify-content-between">
          <p class="head-line"><span>{{ $t('roomTypes.heading') }}</span><spinner v-if="!loaded" /></p>
          <search-filter v-model="filter" :placeholder="$t('roomTypes.filterTip')"/>
        </div>
          ...
          <table class="w-100">
            <thead>
              <tr>
                ...
                <th class="w-name">{{ $t('roomTypes.headersName') }}</th>
                <th class="w-number text-center">{{ $t('roomTypes.headersNumber') }}</th>
                <th class="w-occupancy text-center">{{ $t('roomTypes.headersOccupancy') }}</th>
                <th class="w-valid-until text-center">{{ $t('roomTypes.headersValiduntil') }}</th>
                <th class="w-blockout-days text-center">{{ $t('roomTypes.headersBlockoutDays') }}</th>
            ...
            <tbody v-if="noRooms">
              <tr>
                <td colspan="6" class="w-empty">{{ $t('roomTypes.noRooms') }}</td>
              </tr>
            </tbody>
        ...
        <div v-if="loaded">
          <b-btn pill variant="outline-primary" class="add-new-plan"
            ...
            <icon width="10" height="11" type="plus"/>
            {{ $t('roomTypes.buttonAdd') }}
          </b-btn>
          <p class="bottom-tip">{{ $t('roomTypes.footer') }}</p>
        </div>
      </div>

      <p class="head-line d-md-none"><span>{{ $t('roomTypes.heading') }}</span><spinner v-if="!loaded" /></p>
      <div class="d-md-none list-item" v-if="noRooms">
        <div class="w-empty">{{ $t('roomTypes.noRooms') }}</div>
      </div>
      <div class="d-md-none list-item" v-for="row in rooms" :key="row.id">
        <div class="d-flex">
          <div class="c-1">
            <p class="label">{{ $t('roomTypes.headersName') }}</p>
        ...
        <div class="d-flex line">
          ...
          <div class="w-33">
            <p class="label">{{ $t('roomTypes.headersNumber') }}</p>
            <p>{{ row.amount }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('roomTypes.headersOccupancy') }}</p>
            <p>{{ occupancyForRoom(row) }}</p>
          </div>
        </div>
        <div class="d-flex line">
          <div class="w-33">
            <p class="label">{{ $t('roomTypes.headersValiduntil') }}</p>
            <p>{{validityUntilDate(row.validity.until) }}</p>
          </div>
          <div class="w-33">
            <p class="label">{{ $t('roomTypes.headersBlockoutDays') }}</p>
    ...
    computed: {
      ...
      modalTitle() {
        if (this.room.id == null) {
          return this.$t('roomTypes.titleAdd');
        }
        const { id, langs: { en: { name } } } = this.room;
        return this.$t('roomTypes.titleEdit', { id, name });
      },
      filteredRows() {
