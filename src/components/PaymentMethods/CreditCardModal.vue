    ...
    <div class="content-modal credit-content">
      ...
      <ValidationObserver ref="form" slim v-slot="{ handleSubmit }">
        <b-form @submit.prevent.stop="handleSubmit(submitPaymentMethod)" novalidate>
          <label class="text-md">
            {{ $t('payments.creditCardFormLabelsCardHolder') }}
          </label>
          <ValidatedField v-model="form.cardHolder"
                          :placeholder="$t('payments.creditCardFormPlaceholdersCardHolder')"
                          rules="required_string"
                          type="text"
                          id="cardHolder"
                          name="cardHolder"/>
          <label class="text-md">
            {{ $t('payments.creditCardFormLabelsCardNumber') }}
          </label>
          ...
          <div class="row">
            <div class="col-6">
              <label class="text-md">
                {{ $t('payments.creditCardFormLabelsExpirationMonth') }}
              </label>
            ...
            <div class="col-6">
              <label class="text-md">
                {{ $t('payments.creditCardFormLabelsCvc') }}
              </label>
    ...
    methods: {
      ...
      async submitPaymentMethod() {
        ...
        if (error) {
          ...
          this.$toastr.s({
            title: this.$t('payments.alertUpdated'),
            msg: '',
            timeout: 3000,
            progressbar: false,
          });
