  ...
  >
    ...
    <ValidationObserver ref="form" slim>
      <div class="edge" :class="{ active: edgeTransmission }">
        <h3 :content="$t('channels.connectPeriod')"></h3>
        <div class="row row-edit">
          <div class="col cell-edit-label double">
            {{ $t('channels.connectTransmissionPeriod') }}
          </div>
        ...
        <div class="row row-edit">
          <div class="col cell-edit-label double">
            {{ $t('channels.connectTransmissionEndDate') }}
          </div>
          <div class="col cell-edit-field cell-trans-until">
            <date-picker id="period-end" v-model="values.period.until" min-today placeholder=" "
                         :disabled="pending" :format="dateFormat"/>
          </div>
        </div>
      </div>
      <div class="edge" :class="{ active: edgeData }">
        <form>
          <h3 :content="$t('channels.connectPropertyData')"></h3>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      modalTitle() {
        return this.$t(`channels.title${this.edit ? 'Update' : 'Activate'}`, { name: this.name });
      },
      timeUnits() {
        return ['d', 'w', 'm', 'y'].map((id) => ({
          id,
          text: this.$tc(`channels.connectTimeUnits${id}`, this.values.period.number),
        }));
      },
      isZeroUnit() {
