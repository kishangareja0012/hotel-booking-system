<template>
  ...
  >
    ...
    <ValidationObserver ref="userForm" slim tag="form" novalidate>
      ...
      <div class="row d-flex justify-content-around" v-if="!isNewUser && !userIsInvited">
        <div class="col-12 col-md-6" v-if="!userIsInvited">
          <label class="text-xs" for="first_name">
            {{ $t('users.modalNewUserFieldNameFirstTitle') }}<span class="required">*</span>
          </label>
          <ValidatedField rules="required_string|max:100" group-class="mb-3" no-icon
                          type="text" name="first_name" id="first_name"
                          :placeholder="$t('users.modalNewUserFieldNameFirstPlaceholder')"
        ...
        <div class="col-12 col-md-6" v-if="!userIsInvited">
          <label class="text-xs" for="las_name">
            {{ $t('users.modalNewUserFieldNameLastTitle') }}<span class="required">*</span>
          </label>
          <ValidatedField rules="required_string|max:100" group-class="mb-3" no-icon
                          type="text" name="last_name" id="last_name"
                          :placeholder="$t('users.modalNewUserFieldNameLastPlaceholder')"
      ...
      <div class="row d-flex justify-content-around">
        <div class="col-12">
          <label class="text-xs" for="user-email">
            {{ $t('users.modalNewUserFieldLoginTitle') }}<span class="required">*</span>
          </label>
          <ValidatedField rules="required|email" group-class="mb-2" no-icon
                          :placeholder="$t('users.modalNewUserFieldLoginPlaceholder')"
        ...
        <div class="col-12" v-if="!isNewUser && !userIsInvited">
          <label class="text-xs" for="tel">
            {{ $t('users.phoneNumber') }}<span class="required">*</span>
          </label>
      ...
      <div class="row">
        <div class="col-md-6 col-sm">
          <p class="head-line">{{ $t('users.modal.newUser.field.language.title') }}</p>
          <drop-down
            name="user-lang"
            class="mb-0"
            v-model="user.lang"
      ...
      -->
      <hr class="line-separator" v-if="!isNewUser  && !userIsInvited">
      <div class="row d-flex justify-content-around" v-if="!isNewUser  && !userIsInvited">
        <div class="col-12 col-md-6">
          <label class="text-xs" for="password">
            {{ $t('users.modalNewUserFieldPasswordTitle') }}<span class="required">*</span>
          </label>
        ...
        <div class="col-12 col-md-6">
          <label class="text-xs" for="password">
            {{ $t('users.modalNewUserFieldPasswordTitleConfirm') }}<span class="required">*</span>
          </label>
          <ValidatedField :rules="rulesForPassword(true)"
                          ...
                          :placeholder="$t('resetPwd.passwordConfirm')"  group-class="mb-2"
                          :disabled="pending"
          />
        </div>
      </div>
      ...
      <div class="row" v-if="!groupOwner">
        <div class="col-12 col-md-12">
          ...
          <label class="text-xs">{{ $t('users.modalNewUserPropertiesAccess') }}</label>
          <radio v-model="user.all_group_hotels" name="agh" :val="false" class="mb-2"
                 :disabled="pending">
            {{ $t('users.modalNewUserPermsByRoles') }}
            <icon id="users-perms-roles" type="info" h="16" w="16" class="ml-1" />
            <b-tooltip target="users-perms-roles">
              {{ $t('users.modalNewUserPermsByRolesTip') }}
            </b-tooltip>
          </radio>
          <radio v-model="user.all_group_properties" name="agh" :val="true" class="mb-2"
                 :disabled="pending">
            {{ $t('users.modalNewUserPermsAllHotels') }}
            <icon id="users-perms-all" type="info" h="16" w="16" class="ml-1" />
            <b-tooltip target="users-perms-all">
              {{ $t('users.modalNewUserPermsAllHotelsTip') }}
            </b-tooltip>
          </radio>
          <hr class="line-separator"> -->
        ...
        <!-- <div class="col-12 col-md-6">
          <label class="text-xs">
            {{ $t('users.modalNewUserDefaultPerms')  }}:
            <icon id="users-perms-default" type="info" h="14" w="14" class="ml-1" />
            <b-tooltip target="users-perms-default">
              {{ $t('users.modalNewUserDefaultPermsTip') }}
            </b-tooltip>
          </label>
          <pages-selector mode="property" v-model="user.pages" :allowed="allowedPages" show-all
                          :disabled="pending" />
        </div> -->
      </div>
    </ValidationObserver>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      userIsInvited() {
        return !this.isNewUser && this.user?.profile === null;
      },
      editFields() {
        return ['password', 'password_confirmation', 'profile'];
      },
      modalTitle() {
        if (this.groupOwner && this.isNewUser) return this.$t('users.modalNewUserTitleAddOwner', { group: this.group?.name });
        if (this.isNewUser) return this.$t('users.modalNewUserTitleAdd');
        return this.$t('users.modalNewUserTitleEdit');
      },
      userFormInvalid() {
