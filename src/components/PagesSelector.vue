    ...
    <template v-for="({ name, cat, sub }) in allowedPages">
      <div class="item" :key="`page-${name}`">
        <p :class="{ 'text-disabled': disabled }">
          <span v-if="sub">•</span>{{ !cat ? $t(`permissions.${name}Title`) : $t(`permissions.menu${name}`) }}
        </p>
  ...
  export default {
    ...
    props: {
      ...
      mode: {
        type: String,
        required: true,
        validator: (v) => ['group', 'user', 'property'].includes(v),
      },
      allowed: {
        required: true,
    ...
    computed: {
      ...
      allowedPages() {
        const ret = [];
        const { mode } = this;
        const a = this.pages.filter(({ name, for_property: fh }) => (
          this.allowed.includes(name) && (
            mode === 'group' || (mode === 'user' && !fh) || (mode === 'property' && fh)
          )
        ));
        let cat = null;
